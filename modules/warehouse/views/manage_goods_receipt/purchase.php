	...
	<div class="content">
		<div class="row">
			<div class="col-md-12" id="small-table">
				<div class="panel_s">
					<?php echo form_open_multipart(admin_url('warehouse/manage_goods_receipt'), ['id' => 'add_goods_receipt']); ?>
...
?>
						...
						<div class="row">
							<div class="col-md-6">
								...
								<?php echo render_input('goods_receipt_code', 'stock_received_docket_number', $goods_receipt_code, '', ['disabled' => 'true']); ?>
							</div>
							<div class="col-md-3">
...
echo render_input('deliver_name', 'deliver_name', $deliver_name); ?>
							...
							<div class="col-md-3 ">
								...
								<?php echo render_select('warehouse_id_m', $warehouses, ['warehouse_id', 'warehouse_name'], 'warehouse_name', $warehouse_id_value); ?>
							</div>
							<div class="col-md-3 ">
								<?php $warehouse_id_value = (isset($goods_receipt) ? $goods_receipt->warehouse_id : ''); ?>
								<!-- <a href="#" class="pull-right display-block input_method"><i class="fa fa-question-circle skucode-tooltip"  data-toggle="tooltip" title="" data-original-title="<?php echo _l('goods_receipt_warehouse_tooltip'); ?>"></i></a> -->
								<?php echo render_select('storage_location_id_m', $location, ['id', ['warehouse_code', 'location']], 'Storage location'); ?>
							</div>
							<?php if (ACTIVE_PROPOSAL == true) { ?>
					...
					<div class="row">
						<div class="col-md-12 mtop15">
							<div class="panel-body ">
								...
								<?php echo render_textarea('description', 'note', $description, [], [], 'mtop15'); ?>
...
{ 
  ...
  var $sel = $('select[name="warehouse_id_m"]');
  var admin_url = '<?php echo admin_url(); ?>';
  var url = admin_url + 'warehouse/warehouse/get_warehouse_locations';
  $sel.on('change', function() {  
    var warehouse = this.value;
     if(warehouse){
      ...
      data    : { warehouse: warehouse },
      success : function(data) {   
        var $select = $('#storage_location_id_m');
$select.find('option').remove();   
...
{ 
     $select.append('<option value=' + value.id + '>' + value.storage_location + '</option>');
     $('#storage_location_id_m').selectpicker("refresh"); 
}); 
} else { $select.empty(); $('#storage_location_id_m').selectpicker("refresh"); 
