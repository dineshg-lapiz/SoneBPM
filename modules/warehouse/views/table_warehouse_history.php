...
$aColumns = [
    ...
    db_prefix().'wh_loss_adjustment.storage_location as sl',
...
/* Added code for Add  Location start*/
    $str = '';
    if ($aRow['sl'] != '' && $aRow['sl'] != '0') {
        $team = get_storage_location_name($aRow['sl']);
        if ($team) {
            $value = $team != null ? get_object_vars($team)['storage_location'] : '';
            if ($value != '') {
                $row[] = '<span class="label label-tag tag-id-1"><span class="tag">'.$value.'</span><span class="hide">, </span></span>&nbsp';
            } else {
                $row[] = '';
            }
