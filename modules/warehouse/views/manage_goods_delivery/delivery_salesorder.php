   ...
   <div class="content">
      <div class="row">
         <div class="col-md-12" id="small-table">
            <div class="panel_s">
                  <?php echo form_open_multipart(admin_url('warehouse/so_to_dc'), array('id'=>'add_goods_delivery')); ?>
                ...
                <div class="row" >
                     ...
                     <div class="row">
                        ...
                        <div class="col-md-3">
                          <?php $date_c = isset($goods_delivery) ? $goods_delivery->date_c : $current_day ;?>
                            ...
                            <?php echo render_date_input('date_c','accounting_date', _d($current_day), $disabled) ?>

                        </div>
                        <div class="col-md-3">
                          <?php $date_add = isset($goods_delivery) ? $goods_delivery->date_add : $current_day ;?>
                          <?php echo render_date_input('date_add','day_vouchers', _d($current_day), $disabled) ?>
                        </div>
						...
						<div class="col-md-6  ">
                         <div class="form-group">
                          <label for="rel_to"><?php echo _l('related_to'); ?></label>
                            <select onchange="change_related_to(this.value);" name="rel_to" id="rel_to" class="selectpicker"  data-width="100%">
						...
						<div class="col-md-6 rel_to_inv <?php if($pr_orders_status == true && get_warehouse_option('goods_delivery_required_po') == 1){ echo 'hide';} ;?> ">
                         <div class="form-group">
						...
						<div class="col-md-6 rel_to_so" style="display:none;">
                         <div class="form-group">
                          <label for="invoice_id"><?php echo _l('salesorder'); ?></label>
                            <select onchange="salesorder_change(this); return false;" name="so_id" id="so_id" class="selectpicker" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" <?php if($edit_approval == 'true'){ echo 'disabled';} ; ?> >
                                <option value=""></option>
                                <?php foreach($salesorders as $invoice) { ?>
                                <option value="<?php echo html_entity_decode($invoice['id']); ?>" <?php if(isset($so_id) && $so_id == $invoice['id']){ echo 'selected'; } ?>><?php echo format_salesorder_number($invoice['id']).' - '.$invoice['company'].' - '.$invoice['name']; ?></option>
                                  <?php } ?>
                            </select>
                          </div>
                        </div>
                        ...
                        <!--<div class="col-md-6 <?php if($pr_orders_status == true && get_warehouse_option('goods_delivery_required_po') == 1){ echo 'hide';} ;?> ">
                         <div class="form-group">
                          <label for="invoice_id"><?php echo _l('salesorder'); ?></label>
                            ...
                            <select onchange="salesorder_change(this); return false;" name="invoice_id" id="invoice_id" class="selectpicker" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" <?php if($edit_approval == 'true'){ echo 'disabled';} ; ?> >
                                <option value=""></option>
                                <?php foreach($invoices as $invoice) { ?>
                                <option value="<?php echo html_entity_decode($invoice['id']); ?>" <?php if(isset($so_id) && $so_id == $invoice['id']){ echo 'selected'; } ?>><?php echo format_salesorder_number($invoice['id']).' - '.$invoice['company'].' - '.$invoice['name']; ?></option>
                                  <?php } ?>
                  ...
                  <?php } ?>
                    ...
                    <div class="panel-body mtop10 invoice-item">
                      <div class="row">
                        <div class="col-md-4">
                          <?php //print "<pre>";print_r($items);exit;
						  $this->load->view('warehouse/item_include/main_item_select'); ?>
                        </div>
...
function change_related_to(val){
	 if(val == 'invoice') {
		$('.rel_to_inv').show(); 
		$('.rel_to_so').hide(); 
	} else {
...
<script type="text/javascript">
<?php if(isset($related_to) && $related_to=="saleorder"){ ?>
	$('#rel_to').val('saleorder');
	change_related_to('saleorder');
<?php } ?>
</script>
<?php if(isset($so_id)){ ?>
<script type="text/javascript">
call_salesorder();
function call_salesorder(){
  "use strict";

  var invoice_id = $('select[name="so_id"]').val();
//alert(admin_url + 'warehouse/copy_salesorder/'+invoice_id);
		var csrfName = $('.txt_csrfname').attr('name'); // Value specified in $config['csrf_token_name']
		var csrfHash = $('.txt_csrfname').val(); // CSRF hash
  $.post(admin_url + 'warehouse/copy_salesorder/'+invoice_id,{[csrfName]: csrfHash}).done(function(response){
//alert("123");
