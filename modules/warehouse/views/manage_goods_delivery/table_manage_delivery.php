        ...
        if($aColumns[$i] == 'customer_code'){
            ...
            if($aRow['customer_code']){
				...
				if($aRow['invoice_id']){
					...
					if (str_contains($aRow['invoice_no'], 'SO')) { 
					}else{
				...
				if($aRow['invoice_no']){
					...
					if (str_contains($aRow['invoice_no'], 'SO')) { 	
				    	$_data = format_salesorder_number($aRow['invoice_id']).get_invoice_company_projecy($aRow['invoice_id']).$type_of_delivery;
					}
