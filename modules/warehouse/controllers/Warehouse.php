...
//print "<pre>";print_r($data);
			...
			if (!$this->input->post('id')) {//print "111";
		...
		//$data['salesorders'] = $this->warehouse_model->get_salesorder();
		$this->load->model('salesorder_model');
		//$data["salesorders"] = $this->salesorder_model->get();
		$data["salesorders"] = $this->warehouse_model->get_salesorder();
		...
		if($id != ''){
			...
			if (count($data['goods_delivery_detail']) > 0) {
				$index_receipt = 0;
				//print "<pre>";print_r($data['goods_delivery_detail']);exit;
	...
	public function so_to_dc($id ='', $edit_approval = false) {
		...
		if ($this->input->post()) {
			...
			$this->load->model('salesorder_model');
			$invoice = $this->salesorder_model->get($this->input->post('id'));
			...
			foreach ($so_data as $item) {
				if(count($so_data)==count($invoice_data["items"])){
					/*print $item["id"]."==".$invoice_data["items"][$k]["id"]." && ".$item["qty"].">".$invoice_data["items"][$k]["quantities"];
					exit;*/
					if($item["id"]==$invoice_data["items"][$k]["id"] && $invoice_data["items"][$k]["quantities"]>$item["qty"]){
						//print "sadsadsa";exit;
						set_alert('warning', _l('DC Quantity is more than Sale Order Quantity'));
						redirect(admin_url('warehouse/so_to_dc/'.$this->input->post('id')));
					}elseif($item["id"]==$invoice_data["items"][$k]["id"] && $invoice_data["items"][$k]["quantities"]<$item["qty"]){
						//print "sadsadsa";exit;
						$update_data["status"]="Partially Delivered";
						//$this->salesorder_model->update($update_data, $id);
						$this->db->where('id', $this->input->post('id'))->update('salesorder', $update_data);
					...
					}elseif($item["id"]==$invoice_data["items"][$k]["itemid"] && $invoice_data["items"][$k]["quantities"]==$item["qty"]){
						//print "sadsadsa";exit;
...
print "here";exit;*/
			if ($this->input->post('id')) {
				$mess = $this->warehouse_model->add_goods_delivery_salesorder($data);
				//print "here";exit;
				if ($mess) {
					...
					$update_data["dc_id"]= $invoice->dc_id.",".$mess;
					$update_data["dc_date"]=date("Y-m-d h:i:s");
					$this->db->where('id', $this->input->post('id'))->update('salesorder', $update_data);
					/*print $this->db->last_query();
					exit;*/
					
					$invoice_qty = $this->salesorder_model->get($this->input->post('id'));
					$all_invoices = explode(",",$invoice_qty->dc_id);
					...
					foreach ($so_data as $item) {
						...
						if($invoice_qty->dc_id!=""){
							$exp_inv = explode(",",$invoice_qty->dc_id);
							$exp_inv = array_filter(array_values($exp_inv));
							$inv_item_qty = [];
							//print "<pre>";print_r($exp_inv);
							foreach($exp_inv as $e_inv){
								$inv_item = (array)$this->warehouse_model->get_goods_delivery_detail($e_inv);
								
								foreach ($inv_item as $k=>$itm) {
									$inv_item_qty[$k] += $itm["quantities"];
									//$n++;
								}
...
//print "<pre>";print_r($data['items']);exit;
		...
		$data['customer_code'] = $this->clients_model->get();
		/*if($edit_approval){
			$invoices_data = $this->db->query('select *, iv.id as id from '.db_prefix().'salesorder as iv left join '.db_prefix().'projects as pj on pj.id = iv.project_id left join '.db_prefix().'clients as cl on cl.userid = iv.clientid  order by iv.id desc')->result_array();
			$data['invoices'] = $invoices_data;
		}else{
			//$data['invoices'] = $this->warehouse_model->get_salesorder();
			$this->load->model('salesorder_model');
			$data["invoices"] = $this->salesorder_model->get();
		}*/
		...
		$goods_delivery = $this->salesorder_model->get($id);
		$goods_delivery->address = $goods_delivery->client->address;
		$data["goods_delivery"] = $goods_delivery;
		$data["so_id"] = $id;
	//print "<pre>";print_r($data['salesorders']);exit;
		$data['goods_code'] = $this->warehouse_model->create_goods_delivery_code();
		$data['staff'] = $this->warehouse_model->get_staff();
		$data['current_day'] = date('Y-m-d');
		...
		if($id != ''){
			...
			$data['goods_delivery_detail'] = $this->warehouse_model->get_goods_delivery_detail($id);
			$goods_delivery->goods_delivery_code='';
			$data['goods_delivery'] = $goods_delivery;
//$invoice->items       = get_items_by_type('salesorder', $id);
			if(isset($goods_delivery->pr_order_id ) && (float)$goods_delivery->pr_order_id > 0){
				$is_purchase_order = true;
			}
			$data["so_items"]  = $this->warehouse_model->get_saleorder_items_by_type('salesorder', $id); // = $so_data_items;
			
			
			$so_data = $data["so_items"];
			//print "<pre>";print_r($so_data);
			$index=0;
			foreach ($so_data as $item) {
				//print "<pre>";print_r($item);
				if($invoice->dc_id!=""){
					$exp_inv = explode(",",$invoice->dc_id);
					$exp_inv = array_filter(array_values($exp_inv));
					$inv_item_qty = [];
					//print "<pre>";print_r($exp_inv);
					foreach($exp_inv as $e_inv){
			...
			//print "<pre>";print_r($so_data_items);exit;
			$data["so_items"] = $so_data_items;
...
//print "<pre>";print_r($data['so_items']);exit;
			if (count($data['items']) > 0) {
				$index_receipt = 0;
				foreach ($data['so_items'] as $delivery_detail) {
					//print "<pre>";print_r($delivery_detail);exit;
					//if($delivery_detail['commodity_code'] != null && is_numeric($delivery_detail['commodity_code'])){
						$index_receipt++;
						$unit_name = '';//wh_get_unit_name($delivery_detail['unit_id']);
						$taxname = '';
						$expiry_date = null;
						$lot_number = null;
						$commodity_name = $delivery_detail['description'];
						$without_checking_warehouse = 0;
						...
						$get_commodity = $this->warehouse_model->get_commodity($delivery_detail['id']);
						$taxes   = $this->warehouse_model->get_invoice_item_taxes($delivery_detail['id']);
						/*if($get_commodity){
							$without_checking_warehouse = $get_commodity->without_checking_warehouse;
						}*/
//print "<pre>";print_r($$taxes);exit;
						$goods_delivery_row_template .= $this->warehouse_model->create_goods_delivery_row_template($warehouse_data, 'items[' . $index_receipt . ']', $commodity_name, '', '', $delivery_detail['qty'], $unit_name, $delivery_detail['rate'], $taxname, $delivery_detail['item_id'], $delivery_detail['unit_id'] , $delivery_detail['tax_rate'], '','', '', '','', $expiry_date, $lot_number, '', '',$delivery_detail['tax_name'],$delivery_detail['tax_id'], $delivery_detail['id'], true, $is_purchase_order, '', $without_checking_warehouse);
...
//print "<pre>";print_r($warehouse_data);exit;
						$goods_delivery_row_template .= $this->warehouse_model->create_goods_delivery_row_template($warehouse_data, 'items[' . $index_receipt . ']', $commodity_name, $delivery_detail['warehouse_id'], $delivery_detail['available_quantity'], $delivery_detail['quantities'], $unit_name, $delivery_detail['unit_price'], $taxname, $delivery_detail['commodity_code'], $delivery_detail['unit_id'] , $delivery_detail['tax_rate'], $delivery_detail['total_money'], $delivery_detail['discount'], $delivery_detail['discount_money'], $delivery_detail['total_after_discount'],$delivery_detail['guarantee_period'], $expiry_date, $lot_number, $delivery_detail['note'], $delivery_detail['sub_total'],$delivery_detail['tax_name'],$delivery_detail['tax_id'], $delivery_detail['id'], true, $is_purchase_order, $delivery_detail['serial_number'], $without_checking_warehouse);
		...
		$data['edit_approval'] = $edit_approval;
		$data['goods_delivery_row_template'] = $goods_delivery_row_template;
		//print "<pre>";print_r($data['goods_delivery_row_template']);exit;
		$get_base_currency =  get_base_currency();
		if($get_base_currency){
			$data['base_currency_id'] = $get_base_currency->id;
		}else{
		...
		$data["related_to"] = "saleorder";
		$this->load->view('manage_goods_delivery/delivery_salesorder', $data);
		//$this->load->view('manage_goods_delivery/delivery', $data);
    ...
    public function copy_invoices($invoice_id = '') {
		...
		//print "<pre>";print_r($invoices_detail);exit;
    	echo json_encode([
	...
	public function copy_salesorder($invoice_id = '') {
    	...
    	$invoices_detail = $this->warehouse_model->copy_salesorder($invoice_id);
		
    	if($invoice_id != ''){
    		$invoice_no = format_salesorder_number($invoice_id);
    	}else{
    		$invoice_no = '';
    	}
		//print "<pre>";print_r($invoices_detail);exit;
	...
	//Code for grouping
	public function get_grouping_variation_html_add(){
    	$variation_html = $this->warehouse_model->get_variation_html('');
    	// $item_html = $this->warehouse_model->get_list_parent_item(['id' => '']);
    	...
    	$parent_data = $this->load->view('item_include/item_select', ['ajaxItems' => $data['ajaxItems'], 'items' => $data['items'] , 'select_name' => 'grouping_parent_id', 'id_name' => 'grouping_parent_id', 'data_none_selected_text' => '', 'label_name' => 'parent_item'], true);
	...
	public function get_grouping_items($row){
    	$variation_html = $this->warehouse_model->get_variation_html('');
    	$item_html = $this->warehouse_model->get_list_parent_item(['id' => '']);
    	...
    	$parent_data = $this->load->view('item_include/item_select', ['ajaxItems' => $data['ajaxItems'], 'items' => $data['items'] , 'select_name' => 'grouping_parent_id_'.$row, 'id_name' => 'grouping_parent_id_'.$row, 'data_none_selected_text' => '', 'label_name' => 'parent_item'], true);
	...
	public function grouping_add_edit($id = '') {
		$data = $this->input->post();

		if ($data) {

			if (isset($data['grouping_parent_id'])) {
				
				//print "<pre>";print_r($data);//exit;
				foreach ( $data['formdata'] as $key => $value) {
					
					if($value['name'] == 'grouping_parent_id'){
						$data['grouping_parent_id'] = $value['value'];
					}
					if (str_contains($value['name'], 'child')) {
						$data['child'][] = $value['value'];
					}
					if (str_contains($value['name'], 'quantity')) {
						$data['quantity'][] = $value['value'];
					}
				}
				//print "<pre>";print_r($data);exit;
//print count($data['child']);exit;
				//$data['long_descriptions'] = $this->input->post('long_descriptions', false);
				if(isset($data['child']) && count($data['child'])>0){
					$result = $this->warehouse_model->add_grouped_products($data);
					//exit;
					if ($result) {
						echo json_encode([ 
							'grouping_parent_id' => $data['grouping_parent_id'],
							'result' => 'success',
						]);		
						die;
						/*set_alert('success',_l('assigned_successfully'));
						redirect(admin_url('warehouse/commodity_list'));*/
						//die;
    ...
    public function get_child_from_parent_item($id)
    {
    	...
    	$variation_html = $this->warehouse_model->get_child_from_parent_item($id);
...
if(strlen($data['inventory_filter']) > 0){
	...
	public function save_checkin()
    {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
			...
			$id = $this->warehouse_model->add_checkin_data($data);
			if ($id) {
				echo "success";
				die;
			}
	...
	public function save_checkout()
    {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
			...
			$id = $this->warehouse_model->update_checkout_data($data);
			if ($id) {
				echo "success";
				die;
			}
