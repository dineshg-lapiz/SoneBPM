...
class Warehouse_model extends App_Model {
	...
	public function add_goods_delivery_salesorder($data, $id = false) {
		$goods_deliveries = [];
		if (isset($data['items'])) {
			$goods_deliveries = $data['items'];
			unset($data['items']);
		}
		$data['invoice_id']=$data['so_id'];
		//unset($data['items']);
		unset($data['isedit']);
		unset($data['id']);
		unset($data['rel_to']);
	...
	public function get_saleorder_items_by_type($type, $id)
	{
		$CI = &get_instance();
		/*$CI->db->select('it.*, i.id as item_id');
		$CI->db->from(db_prefix() . 'itemable as it, '.db_prefix() . 'items as i');
		$CI->db->where('it.rel_id', $id);
		$CI->db->where('it.rel_type', $type);
		$CI->db->where('it.description', 'i.description');
		$CI->db->order_by('it.item_order', 'asc');*/
		//print 'select it.*, i.id as item_id from '.db_prefix() . 'itemable as it, '.db_prefix() . 'items as i where it.rel_id='.$id.' and it.rel_type='.$type.' and it.description=i.description ORDER BY it.item_order asc';
		return $this->db->query('select it.*, i.id as item_id from '.db_prefix() . 'itemable as it, '.db_prefix() . 'items as i where it.rel_id="'.$id.'" and it.rel_type="'.$type.'" and it.description=i.description ORDER BY it.item_order asc')->result_array();
     ...
     * @return array      
     */
    public function  get_salesorder($id = false)
    {
    		...
    		$array =  $this->db->get(db_prefix() . 'salesorder')->result_array();
			return $array;*/
			if (is_numeric($id)) {
				$this->db->where('id', $id);
	
				return $this->db->get(db_prefix() . 'salesorder')->row();
			}
			if ($id == false) {
				/*$arr_invoice = $this->get_invoices_goods_delivery('invoice');
				print "<pre>";print_r($arr_invoice);
				if(count($arr_invoice) > 0){
					return $this->db->query('select *, iv.id as id from '.db_prefix().'salesorder as iv left join '.db_prefix().'projects as pj on pj.id = iv.project_id left join '.db_prefix().'clients as cl on cl.userid = iv.clientid  where iv.id NOT IN ('.implode(", ", $arr_invoice).') order by iv.id desc')->result_array();
				}*/
				return $this->db->query('select *, iv.id as id from '.db_prefix().'salesorder as iv left join '.db_prefix().'projects as pj on pj.id = iv.project_id left join '.db_prefix().'clients as cl on cl.userid = iv.clientid  order by iv.id desc')->result_array();
			}
	...
	public function copy_salesorder($invoice_id)
    {
    	
    	$this->db->where('id', $invoice_id);
    	$invoice_value = $this->db->get(db_prefix().'salesorder')->row();
		//print "<pre>";print_r($invoice_value);//exit;
    	$data_insert=[];
    	$status = false;
    	$goods_delivery_row_template = '';
    	$goods_delivery_row_template = $this->warehouse_model->create_goods_delivery_row_template();
		...
		//print "<pre>";print_r($$data_insert);exit;
    	$data['additional_discount'] 	= $additional_discount;
    	$data_insert['goods_delivery'] = $data;
    	$data_insert['goods_delivery_detail'] = $goods_delivery_row_template;
    	$data_insert['status'] = $status;
//print "<pre>";print_r($data_insert);exit;
    ...
    public function create_goods_delivery_row_template($warehouse_data = [], $name = '', $commodity_name = '', $warehouse_id = '', $available_quantity = '', $quantities = '', $unit_name = '', $unit_price = '', $taxname = '',  $commodity_code = '', $unit_id = '', $tax_rate = '', $total_money = '', $discount = '', $discount_money = '', $total_after_discount = '', $guarantee_period = '', $expiry_date = '', $lot_number = '', $note = '',  $sub_total = '', $tax_name = '', $tax_id = '', $item_key = '',$is_edit = false, $is_purchase_order = false, $serial_number = '', $without_checking_warehouse = 0) {
		...
		if ($name == '') {
			...
			if($is_edit){
				//$invoice_item_taxes = wh_convert_item_taxes($tax_id, $tax_rate, $tax_name);
				$invoice_item_taxes = $this->get_invoice_item_taxes($item_key);
				$arr_tax_rate = explode('|', $tax_rate);
				foreach ($arr_tax_rate as $key => $value) {
					$tax_rate_value += (float)$value;
				}
...
//print "<pre>";print_r($invoice_item_taxes);exit;
			if((float)$tax_rate_value != 0){
				$tax_money = (float)$unit_price * (float)$quantities * (float)$tax_rate_value / 100;
				$goods_money = (float)$unit_price * (float)$quantities + (float)$tax_money;
				$amount = (float)$unit_price * (float)$quantities + (float)$tax_money;
...
//print "<pre>";print_r($tax_name);
	...
	public function add_grouped_products($data) { //print "<pre>";print_r($data);
		$data["child"] = array_values($data["child"]);
		$data["quantity"] = array_values($data["quantity"]);
		//print "<pre>";print_r($data);//exit;
		
		$this->db->where('product_id', $data['grouping_parent_id']);
		$this->db->delete(db_prefix() . 'items_product_grouping');
		
		for($m=0;$m<count($data["child"]);$m++){
			$data_insert['addedfrom'] = get_staff_user_id();
			$data_insert['datecreated'] = date('Y-m-d H:i:s');
					
			$data_insert['product_id'] = $data['grouping_parent_id'];
			$data_insert['sub_product_id'] = $data["child"][$m];
			$data_insert['qty'] = $data["quantity"][$m];
			// status '1:Goods receipt note 2:Goods delivery note',
			$this->db->insert(db_prefix() . 'items_product_grouping', $data_insert);
			//print $this->db->last_query();exit;
		}

		return true;
	}
	public function get_child_from_parent_item($id)
    {
    	//parent_id, item_id
    	$index=0;
		$html = '';
    	...
    	//case has parent id, don't parent_id
				...
				$get_child_html = $this->get_child_html($id);
				$html = $get_child_html['html'];
				$index = $get_child_html['index'];
    	...
    	//$new_html .= '<div class="row">'.$html.'</div>';
		$new_html .= ''.$html.'';
    	return ['html' => $new_html, 'index' => $index];
    }
	public function get_child_html($parent_id)
    {
		$html = '';
		$index = 0;
		$parent_value = $this->get_child_list($parent_id);
		//print "<pre>";print_r($parent_value);exit;
		...
		foreach ($parent_value as $item) {
			...
			$item_html = '';
			$item_html = $this->get_list_parent_child_item(['id' => $item["product_id"], 'child_id' => $item["sub_product_id"]]);
			$item_html = $item_html['item_options'];
			
				$html .= '<div class="child_row"><div class="col-md-6" style="margin:10px 0px;">Child Item :<select id="child'.$index.'" name="child['.$index.']" class="selectpicker" data-width="100%" data-none-selected-text="" data-live-search="true" tabindex="-98">'.$item_html.'</select></div><div class="col-md-4"  style="margin:10px 0px;">Quantity : <input type="number" value="'.$item["qty"].'" class="form-control" name="quantity['.$index.']" id="quantity['.$index.']"/></div><div class="col-md-2" style="padding:28px 0 0 0;"><span class="pull-bot"><button name="add" id="DeleteRow" class="btn btn-danger" data-ticket="true" type="button"><i class="fa fa-minus"></i></button></span></div></div>';
			
		}

		return ['html' => $html, 'index' => $index];
    }
	public function get_child_list($id = false) {

		if (is_numeric($id)) {
			$this->db->where('product_id', $id);

			return $this->db->get(db_prefix() . 'items_product_grouping ')->result_array();
		}
		if ($id == false) {
			return $this->db->query('select * from tblitems_product_grouping ')->result_array();
		}

	}
	public function get_list_parent_child_item($data)
	{
		 $item_options = '';
		 $flag_is_parent = false;
		...
		if(isset($data['id']) && $data['id'] != ''){
			...
			$array_child_value = $this->db->get(db_prefix().'items_product_grouping')->result_array();
	...
	public function add_checkin_data($data){
		unset($data["lat"]);
		unset($data["lon"]);
		//unset($data["csrf_token_name"]);
		date_default_timezone_set("Asia/Kolkata");
		$this->db->insert(db_prefix() . 'delivery_se_logs', array_merge($data, [
            'datecreated' => date('Y-m-d H:i:s'),
            'addedfrom'   => is_staff_logged_in() ? get_staff_user_id() : 0,
			'se_id' => get_staff_user_id(),
			'dc_id' => $data["dc_id"],
	...
	public function verify_checked_id($client_id){
		$this->db->select('*');
        $this->db->from(db_prefix() . 'delivery_se_logs');
        $this->db->where('dc_id', $client_id);
		$this->db->where('se_id', get_staff_user_id());
		$this->db->where('visit_date', date('Y-m-d'));
		$this->db->order_by('id', 'desc');
        $se_data = $this->db->get()->row();
		//print "<pre>";print_r($se_data);exit;
		return $se_data;
	}
	public function se_logs($client_id){
		$this->db->select('*');
        $this->db->from(db_prefix() . 'delivery_se_logs');
        $this->db->where('dc_id', $client_id);
		//$this->db->where('se_id', get_staff_user_id());
		//$this->db->where('visit_date', date('Y-m-d'));
		//$this->db->order_by('id', 'desc');
        $se_data = $this->db->get()->result_array();
		//print "<pre>";print_r($se_data);exit;
		return $se_data;
	}
	public function all_se_logs(){
		$this->db->select('*');
        $this->db->from(db_prefix() . 'delivery_se_logs');
	...
	public function update_checkout_data($data){
		unset($data["lat"]);
		unset($data["lon"]);
		unset($data["csrf_token_name"]);
		$se_log_id = $data["se_log_id"];
		unset($data["se_log_id"]);
		$checked_in = $this->verify_checked_id($data["dc_id"]);
		
		date_default_timezone_set("Asia/Kolkata");
		
		$from_time = strtotime($checked_in->check_in_time); 
		$to_time = strtotime(date('Y-m-d H:i:s')); 
		$diff_minutes = round(abs($from_time - $to_time) / 60,2);
		//print "<pre>";print_r($data);print $diff_minutes;
		$this->db->where('id', $se_log_id);
		$data = array_merge($data, [
			'dateupdated' => date('Y-m-d H:i:s'),
			'updatedfrom' => is_staff_logged_in() ? get_staff_user_id() : 0,
			'checked_in' => "0",
			'check_out_time' => date('Y-m-d H:i:s'),
			'duration' => $diff_minutes,
        ]);
        $this->db->update(db_prefix() . 'delivery_se_logs', $data);
