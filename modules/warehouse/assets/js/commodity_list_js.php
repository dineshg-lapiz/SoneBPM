...
warehouse_type_value = warehouse_type;
  ...
  var expenseDropzone;
   ...
   function expenseSubmitHandler(form){
    ...
    data.hsn_code = $('input[name="hsn_code"]').val();
  ...
  //Code for grouping
  function commodity_grouping(){
    "use strict";
	$.post(admin_url + 'warehouse/get_grouping_variation_html_add').done(function(response) {
      response = JSON.parse(response);
        //variation value
        $('.list_approve').html('');
        $('.list_approve').append(response.variation_html);
        ...
        init_ajax_search('items','#grouping_parent_id.ajax-search',undefined,admin_url+'warehouse/wh_parent_item_search');
    });
    $('#grouping_add_edit').modal('show');
	$('.add_child_grouping').hide();
  }
	$("body").on('change', 'select[name="grouping_parent_id"]', function () {
		var parent_id = $('select[name="grouping_parent_id"]').val();
		$('.add_child_grouping').show();
		
		$.post(admin_url + 'warehouse/get_child_from_parent_item/'+parent_id).done(function(response) {
		  response = JSON.parse(response);
	//alert(response);
		  //variation value
		  //$('.list_approve').html('');
		  $('.child_list').html('');
		  $('.child_list').append(response.variation_html);
		  $(".selectpicker").selectpicker('refresh');
		});
	...
	$("body").on("click", "#add_item_row", function () {
		if($('.child_row').length>0){
			var counter = $('.child_row').length+1;
		}else{
			var counter = 1;
		}
    	"use strict";
		...
		$.post(admin_url + 'warehouse/get_grouping_items/'+counter).done(function(response) {
		  response = JSON.parse(response);
		  
		  var newRow = $('<div class="child_row"><div class="col-md-6" style="margin:10px 0px;">Child Item :<select required id="child'+counter+'" name="child['+counter+']" class="" data-width="100%" data-none-selected-text="" data-live-search="true" tabindex="-98"></select></div><div class="col-md-4"  style="margin:10px 0px;">Quantity : <input type="number" required class="form-control" name="quantity[' +counter + ']" id="quantity[' +counter + ']"/></div><div class="col-md-2" style="padding:28px 0 0 0;"><span class="pull-bot"><button name="add" id="DeleteRow" class="btn btn-danger" data-ticket="true" type="button"><i class="fa fa-minus"></i></button></span></div></div><style >#child'+counter+'-error{position:absolute;top:34px;}</style>');
		  //alert(response.item_html);
		  
          $('.child_list').append(newRow);
		  //alert(("#child["+counter+"]"));
		  var select_id = "#child"+counter;
		  $(select_id).html(response.item_html).selectpicker('refresh');
		  counter++;
		});
	...
	appValidateForm($("body").find('.grouping_add_edit'), {
      'grouping_parent_id': 'required',
      
    },groupingSubmitHandler);
	
	function groupingSubmitHandler(form){
		"use strict";
		//$('.submit_btn').attr( "disabled", "disabled" );
		var data ={};
		data.grouping_parent_id = $('select[name="grouping_parent_id"]').val();
		var myForm = $(".grouping_add_edit")[0];
		//data.formdata = new FormData(myForm);
		//data.formdata = new FormData(".grouping_add_edit"); /*$( form ).serialize();*/
		//data.formdata = new FormData(document.querySelector("form")); /*$( form ).serialize();*/
		data.formdata = $( form ).serializeArray();
		$.post(form.action, data).done(function(response) {
	//alert(response);
			 var response = JSON.parse(response);
			if (response.result=="success") {
			 if (response.grouping_parent_id) {
				alert_float('success', "<?php echo _l('assigned_successfully') ?>");
					
				$('#grouping_add_edit').modal('hide');
			 ...
			 if (response.result=="failed") {
			 	alert("There are no child items. Please add child using + symbol.");
				return false;
			 }
		  });
		return false;
...
$("body").on('change', 'select[name="parent_id"]', function () {
  ...
  var parent_id = $('select[name="parent_id"]').val();
      ...
      if(!check_id){
          ...
          $('#commodity_list-add-edit input[name="hsn_code"]').val(response.parent_value.hsn_code);
