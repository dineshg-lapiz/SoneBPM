...
foreach ($rResult as $aRow) {
   ...
   for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if($aColumns[$i] == 'total'){
            ...
            if($aRow['delivery_date']!="" && $aRow['delivery_date']<date("Y-m-d")){
				//$_data .= "<script>$(this).css({'border': '2px solid red'});</script>";
				$_data = '<span class="expired">'._d($aRow['delivery_date']).'</span>'."<script>$('.expired').parent().parent().addClass('expired_po');
				//$(this).parent().css({'background-color': 'red !important'});</script>";
			}
