...
{   
    ...
    public function get_purchase_request($id = ''){
        if($id == ''){
            ...
            foreach( $purchase_request as $key => $detail )
            {
                //Code updated by Sooraj For remove `PUR create from` the pur_rq_name field on 16-08-2023
                $purchase_request[$key]['pur_rq_name'] =  str_replace('PUR create from ','',$detail['pur_rq_name']);
            }
            return $purchase_request;
        }else{
            $this->db->where('id',$id);
            $purchase_request = $this->db->get(db_prefix().'pur_request')->row();
            $purchase_request->pur_rq_name =  str_replace('PUR create from ','',$purchase_request->pur_rq_name);
            return $purchase_request;
        }
    }
