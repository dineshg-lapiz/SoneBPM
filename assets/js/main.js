...
$(function () {
  ...
  /** List Salesorder **/
  add_hotkey("Alt+I", function () {
    window.location.href = admin_url + "salesorder/list_invoices";
  });
  /** List Estimates **/
  ...
  table_salesorder = $("table.table-salesorder");

  if (table_invoices.length > 0 || table_estimates.length > 0 || table_salesorder.length > 0) {
...
//alert(table_invoices.length);
	...
	//alert(table_salesorder.length);
	if (table_salesorder.length) {
      // Invoices tables
      initDataTable(
        table_salesorder,
        admin_url +
        ...
        !$("body").hasClass("recurring")
          ...
          : [table_salesorder.find("th.next-recurring-date").index(), "asc"]
      );
    }
  ...
  init_invoices_total();
  // Init salesorder top stats
  init_salesorder_total();
  // Init expenses total
  init_expenses_total();
  // Make items sortable
...
function init_editor(selector, settings) {
  ...
  var _settings = {
    ...
    plugins: [
      ...
      "paste textcolor colorpicker preview",
    ],
    toolbar1:
      "fontselect fontsizeselect | forecolor backcolor | bold italic | alignleft aligncenter alignright alignjustify | image link | bullist numlist | restoredraft preview",
...
function proposal_convert_template(invoker) {
  ...
  } else if (template == "salesorder") {
    html_helper_selector = "salesorder";
  }else {
    return false;
  }
...
function estimate_convert_template(invoker) {
  ...
  requestGet(
    "estimates/get_salesorder_convert_data/" + estimate_id
  ).done(function (data) {
    $("#convert_helper").html(data);
    $("#convert_to_" + html_helper_selector).modal({
      show: true,
...
function init_salesorder(id) {
  load_small_table_item(
    id,
    "#salesorder",
    "invoiceid",
    "salesorder/get_salesorder_data_ajax",
    ".table-salesorder"
  );
}
...
function add_child_item_to_preview(id) { //alert(id); alert("here");
	//$("table.estimate-items-table tbody").find("tr:gt(0)").remove();
		requestGetJSON("invoice_items/get_child_list/" + id).done(function (response) { 
		if(response && response.length>0){																						   
		for (var i = 0; i < response.length; i++) {
			
		var newRow = $('<tr class="main submain newrow_'+i+'"><td></td><td><textarea name="description" rows="4" class="form-control" placeholder="Description"></textarea></td><td><textarea name="long_description" rows="4" class="form-control" placeholder="Long description"></textarea></td><td><input type="number" name="quantity" min="0" value="1" class="form-control" placeholder="Quantity"><input type="text" placeholder="Unit" data-toggle="tooltip" 612="" data-title="e.q kg, lots, packs" name="unit" class="form-control input-transparent text-right" data-original-title="" title=""></td><td><input type="number" name="rate" class="form-control" placeholder="Rate"></td><td><div class="dropdown bootstrap-select show-tick display-block tax main-tax bs3" style="width: 100%;"><select class="selectpicker display-block tax main-tax" data-width="100%" name="taxname" multiple="" data-none-selected-text="No Tax" tabindex="-98"><option value="SGST|9.00" data-taxrate="9.00" data-taxname="SGST" data-subtext="SGST">9.00%</option><option value="CGST|9.00" data-taxrate="9.00" data-taxname="CGST" data-subtext="CGST">9.00%</option><option value="IGST|18.00" data-taxrate="18.00" data-taxname="IGST" data-subtext="IGST">18.00%</option></select><button type="button" class="btn dropdown-toggle btn-default bs-placeholder" data-toggle="dropdown" role="combobox" aria-owns="bs-select-9" aria-haspopup="listbox" aria-expanded="false" title="No Tax"><div class="filter-option"><div class="filter-option-inner"><div class="filter-option-inner-inner">No Tax</div></div></div><span class="bs-caret"><span class="caret"></span></span></button><div class="dropdown-menu open"><div class="inner open" role="listbox" id="bs-select-9" tabindex="-1" aria-multiselectable="true"><ul class="dropdown-menu inner " role="presentation"></ul></div></div></div></td><td><div class="input-group" id="discount-total"><input type="number" value="0" class="form-control pull-left item-input-discount-percent" min="0" max="100" name="item_discount_percent"></div></td><td class="item-discount-total"><input type="text" name="item_discount_amount" readonly="readonly" class="form-control" placeholder="Amount"></td><td></td><td><button type="button" class="btn pull-right btn-primary" onClick="add_item_to_table(\'add_child_'+i+'\',\'undefined\',true); return false;"><i class="fa fa-check"></i></button><a href="#" class="btn btn-danger pull-left" style="margin:10px 0px;" onclick="delete_item(this,undefined); return false;"><i class="fa fa-trash"></i></a></td></tr>');
		
		tableBody = $("table.estimate-items-table tbody");
        tableBody.append(newRow);
		alert(response[i].description);
		$('.newrow_'+i+' textarea[name="description"]').val(response[i].description);
    $('.newrow_'+i+' textarea[name="long_description"]').val(
      response[i].long_description.replace(/(<|&lt;)br\s*\/*(>|&gt;)/g, " ")
    );

    _set_item_preview_custom_fields_array(response[i].custom_fields);

    $('.newrow_'+i+' input[name="quantity"]').val(response[i].quantity);

    var taxSelectedArray = [];
    if (response[i].taxname && response[i].taxrate) {
      taxSelectedArray.push(response[i].taxname + "|" + response[i].taxrate);
    }
    if (response[i].taxname_2 && response[i].taxrate_2) {
      taxSelectedArray.push(response[i].taxname_2 + "|" + response[i].taxrate_2);
    }

    $(".newrow_"+i+" select.tax").selectpicker("val", taxSelectedArray);
    $('.newrow_'+i+' input[name="unit"]').val(response[i].unit);
    ...
    var baseCurency = $currency.attr("data-base");
    var selectedCurrency = $currency.find("option:selected").val();
    var $rateInputPreview = $('.newrow_'+i+' input[name="rate"]');

    if (baseCurency == selectedCurrency) {
      $rateInputPreview.val(response[i].rate);
    } else {
      var itemCurrencyRate = response[i]["rate_currency_" + selectedCurrency];
      if (!itemCurrencyRate || parseFloat(itemCurrencyRate) === 0) {
        $rateInputPreview.val(response[i].rate);
      } else {
        $rateInputPreview.val(itemCurrencyRate);
...
function clear_item_preview_values(default_taxes) {
  ...
  var last_taxes_applied = $("table.items tbody")
    ...
    .find("select")
    .selectpicker("val");
  var previewArea = $(".main").not(".submain");
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
  sales_type = (getLastPart(window.location.href));
  // If not custom data passed get from the preview
  //alert(data);
  var add_child = "";
  var child_item_row = "";
  if(data.indexOf("add_child_") > -1){
	  add_child = "add_child";
	  child_item_row = data.match(/\d+/);
	  //alert(child_item_row);
	  data = get_child_item_preview_values(child_item_row);
  }else{
  	data =
...
//alert(data.item_discount_percent);
  var table_row = "";
  ...
  get_taxes_dropdown_template(tax_name, data.taxname).done(function (
	...
	if(sales_type=="salesorder"){
		table_row +=
		  '<td><input type="number" min="0" readonly data-available_quantity name="newitems[' +
		  item_key +
	...
	/*table_row +=
      '<td><input type="number" min="0" data-item_discount_percent name="newitems[' +
      item_key +
      '][item_discount_percent]" value="' +
      data.item_discount_percent +
      '" class="form-control">';*/
	
	table_row += '<td><div class="input-group" id="discount-total"><input type="number" data-item_discount_percent value="' +data.item_discount_percent +'" onchange="calculate_total();" class="form-control pull-left item-input-discount-percent" min="0" name="newitems[' +item_key +'][item_discount_percent]"></div></td>';
	  
	/*table_row +=
      '<td><input type="number" min="0" data-item_discount_total name="newitems[' +
      item_key +
      '][item_discount_total]" value="' +
      data.item_discount_total +
      '" class="form-control">';*/
	table_row += '<td class="" align="right"><input type="text" readonly data-item_discount_amount value="' +data.item_discount_amount +'" class="form-control pull-left input-item-discount-percent" min="0" name="newitems[' +item_key +'][item_discount_amount]"></td>';
	...
	if(add_child=="add_child"){
		$("table.estimate-items-table tbody .main.newrow_"+child_item_row).remove();
	}
...
function calculate_item_discount(val) {
      if (
        $('select[name="discount_type"]').val() === "" &&
        val != 0
      ) {
...
function get_item_preview_values() {
  ...
  response.available_quantity = $('.main input[name="available_quantity"]').val();
  ...
  response.item_discount_amount = $('.main input[name="item_discount_amount"]').val();
  
  return response;
}
function get_child_item_preview_values(row) {
  var response = {};
  /*alert("123");
  alert(response.long_description);*/
  
  response.description = $('.main.newrow_'+row+' textarea[name="description"]').val();
  response.long_description = $('.main.newrow_'+row+' textarea[name="long_description"]').val();
  response.available_quantity = $('.main.newrow_'+row+' input[name="available_quantity"]').val();
  response.qty = $('.main.newrow_'+row+' input[name="quantity"]').val();
  response.taxname = $(".main.newrow_"+row+" select.tax").selectpicker("val");
  response.rate = $('.main.newrow_'+row+' input[name="rate"]').val();
  response.unit = $('.main.newrow_'+row+' input[name="unit"]').val();
  
  response.item_discount_percent = $('.main.newrow_'+row+' input[name="item_discount_percent"]').val();
  response.item_discount_amount = $('.main.newrow_'+row+' input[name="item_discount_amount"]').val();
...
//alert(quantity);
	item_discount_percent = $(this).find("[data-item_discount_percent]").val();
//item_discount_percent = $(this).find(".item-input-discount-percent").val();
	  // alert(item_discount_percent); 
		//alert("else");
	if (item_discount_percent != "" && item_discount_percent>0) { 
		disc_val = ($(this).find("td.rate input").val() * quantity) * item_discount_percent/100;
		_amount = accounting.toFixed(
		  $(this).find("td.rate input").val() * quantity - disc_val,
		  app.options.decimal_places
		);
		_amount = parseFloat(_amount);
		$(this).find("[data-item_discount_amount]").val(disc_val);
	}else{
		_amount = accounting.toFixed(
...
//alert(total_discount_calculated);
...
// Delete salesorder attachment
function delete_salesorder_attachment(id) {
  if (confirm_delete()) {
    requestGet("salesorder/delete_attachment/" + id)
      .done(function (success) {
        if (success == 1) {
          $("body")
            .find('[data-attachment-id="' + id + '"]')
...
// Invoices quick total stats
function init_salesorder_total(manual) {
  if ($("#salesorder_total").length === 0) {
    return;
  }
  var _inv_total_inline = $(".invoices-total-inline");
  var _inv_total_href_manual = $(".salesorder-total");

  if (
    $("body").hasClass("salesorder-total-manual") &&
    typeof manual == "undefined" &&
    !_inv_total_href_manual.hasClass("initialized")
  ) {
    return;
  ...
  $.post(admin_url + "salesorder/get_invoices_total", data).done(function (
    response
  ) {
    $("#salesorder_total").html(response);
  });
}
...
// Validate invoice add/edit form
function validate_salesorder_form(selector) {
  selector = typeof selector == "undefined" ? "#salesorder-form" : selector;
  ...
  $("body")
    ...
    .rules("add", {
      remote: {
        url: admin_url + "salesorder/validate_invoice_number",
        type: "post",
        data: {
          number: function () {
            return $('input[name="number"]').val();
      ...
      messages: {
        remote: app.lang.salesorder_number_exists,
      },
    });
}
...
function get_item_quantity(selectedValue){
//call  ajax method to get data from database
//alert(selectedValue);
 $.ajax({
            type: "GET",
            url: admin_url + "salesorder/get_item_quantity",//this  should be replace by your server side method
            data: {com_no: selectedValue }, //this is parameter name , make sure parameter name is sure as of your sever side method
	...
	function checkin(client){
		if (navigator.geolocation) {
			...
			$('input[name="location_user"]').val(pos.coords.latitude +" , " + pos.coords.longitude);
			//alert(pos.coords.latitude);
			if(parseFloat(pos.coords.latitude).toFixed(2)==parseFloat($('#latitude').val()).toFixed(2) && parseFloat(pos.coords.longitude).toFixed(2)==parseFloat($('#longitude').val()).toFixed(2)){
				console.log("123");
				$.ajax({
					type: "POST",
					url: admin_url + "clients/save_checkin",//this  should be replace by your server side method
					data: {
	...
	function checkout(client,se_log_id){
		if (navigator.geolocation) {
			navigator.geolocation.getCurrentPosition(function(pos) {
			$('input[name="location_user"]').val(pos.coords.latitude +" , " + pos.coords.longitude);
			//alert(pos.coords.latitude);
			if(parseFloat(pos.coords.latitude).toFixed(2)==parseFloat($('#latitude').val()).toFixed(2) && parseFloat(pos.coords.longitude).toFixed(2)==parseFloat($('#longitude').val()).toFixed(2)){
				...
				$.ajax({
					type: "POST",
					url: admin_url + "clients/save_checkout",//this  should be replace by your server side method
					data: {
						lat: pos.coords.latitude,
						lon: pos.coords.longitude,
						client_id: client,
