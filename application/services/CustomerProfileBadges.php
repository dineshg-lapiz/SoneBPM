...
class CustomerProfileBadges
{
	...
	public function salesorder()
    {
        ...
        if (!staff_can('view', 'invoices')) {
            $where = get_salesorder_where_sql_for_staff($this->staffId);
            $this->CI->db->where($where);
        }
        $this->CI->db->where_not_in('status', [\Salesorder_model::STATUS_CANCELLED, \Salesorder_model::STATUS_PAID]);
        $this->CI->db->where('clientid', $this->customerId);

        return $this->CI->db->count_all_results('salesorder');
    }
