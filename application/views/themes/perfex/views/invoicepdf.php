...
if($invoice->template_id!=""){
	...
	if (str_contains($content, '{invoice_id}')) {
		$content = str_replace('{invoice_id}', $invoice->id, $content);
	}
	if (str_contains($content, '{invoice_number}')) {
		$content = str_replace('{invoice_number}', '# '.format_invoice_number($invoice->id), $content);
	}
	if (str_contains($content, '{invoice_link}')) {
		$content = str_replace('{invoice_link}', site_url('invoice/' . $invoice->id . '/' . $invoice->hash), $content);
	}
	if (str_contains($content, '{invoice_duedate}')) {
		$content = str_replace('{invoice_duedate}', _d($invoice->duedate), $content);
	}
	if (str_contains($content, '{invoice_date}')) {
		$content = str_replace('{invoice_date}', _d($invoice->date), $content);
	}
	if (str_contains($content, '{invoice_status}')) {
		$content = str_replace('{invoice_status}', '<span style="color:rgb(' . invoice_status_color_pdf($status) . ');text-transform:uppercase;">' . format_invoice_status($invoice->status, '', false) . '</span>', $content);
	}
	if (str_contains($content, '{invoice_sale_agent}')) {
		$content = str_replace('{invoice_sale_agent}', get_staff_full_name($invoice->sale_agent), $content);
	}
	
	$currency = get_currency($invoice->currency);
	
	if (str_contains($content, '{invoice_total}')) {
		$content = str_replace('{invoice_total}', app_format_money($invoice->total, $currency), $content);
	}
	if (str_contains($content, '{invoice_subtotal}')) {
		$content = str_replace('{invoice_subtotal}', app_format_money($invoice->subtotal, $currency), $content);
	}
	if (str_contains($content, '{invoice_amount_due}')) {
		$content = str_replace('{invoice_amount_due}', app_format_money(get_invoice_total_left_to_pay($invoice_id, $invoice->total), $currency), $content);
	}
	...
	if (str_contains($content, '{invoice_short_url}')) {
		$content = str_replace('{invoice_short_url}', get_invoice_shortlink($invoice), $content);
	}
	...
	foreach ($custom_fields as $field) {
		...
		if (str_contains($content, '{' . $field['slug'] . '}')) {
			$content = str_replace('{' . $field['slug'] . '}', get_custom_field_value($invoice->id, $field['id'], 'invoice'), $content);
		}
	}
	...
	$content = str_replace('{invoice_items}', $items_html, $content);
