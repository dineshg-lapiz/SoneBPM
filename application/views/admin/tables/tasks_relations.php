...
if (!$this->ci->input->post('tasks_related_to')) {
    ...
    foreach ($tasks_related_to as $rel_to) {
        ...
        } elseif ($rel_to == 'salesorder') {
            $rel_to_query .= '(rel_id IN (SELECT id FROM ' . db_prefix() . 'salesorder WHERE id=' . $this->ci->db->escape_str($rel_id) . ')';
...
foreach ($rResult as $aRow) {//print "<pre>";print_r($aRow);exit;
    $row = [];
