...
if (has_permission('customers', '', 'view_members') || (isset($role["permissions"]["customers"]) && in_array("view_members",$role["permissions"]["customers"]))) { 
	if(isset($staff_members)){
		foreach ($staff_members as $member) {		
			array_push($where, 'OR ' . db_prefix() . 'clients.userid IN (SELECT customer_id FROM ' . db_prefix() . 'customer_admins WHERE staff_id=' . $member['staffid'] . ')');
		}
	}
}
