...
$aColumns = [
    ...
    '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM ' . db_prefix() . 'taggables JOIN ' . db_prefix() . 'tags ON ' . db_prefix() . 'taggables.tag_id = ' . db_prefix() . 'tags.id WHERE rel_id = ' . db_prefix() . 'salesorder.id and rel_type="invoice" ORDER by tag_order ASC) as tags',
    'duedate',
...
foreach ($rResult as $aRow) {
    ...
    if (is_numeric($clientid) || $project_id) {
        $numberOutput = '<a href="' . admin_url('salesorder/list_salesorder/' . $aRow['id']) . '" target="_blank">' . format_salesorder_number($aRow['id']) . '</a>';
    } else {
        $numberOutput = '<a href="' . admin_url('salesorder/list_salesorder/' . $aRow['id']) . '" onclick="init_salesorder(' . $aRow['id'] . '); return false;">'. format_salesorder_number($aRow['id']) . '</a>';
    }
