...
if (has_permission('proposals', '', 'view_members') || (isset($role["permissions"]["proposals"]) && in_array("view_members",$role["permissions"]["proposals"]))) { 
	if(isset($staff_members)){
		foreach ($staff_members as $member) {		
			array_push($where, ' OR (' . db_prefix() . 'proposals.addedfrom=' . $member['staffid'] . ')');
		}
	}
}
