...
<!--<div class="panel_s no-padding mbot5 mergeable-invoices">
    ...
    <div class="panel-body">
        ...
        <div class="tw-flex tw-justify-between tw-items-center tw-mb-2 last:tw-mb-0">
            <div>
                <a href="<?php echo admin_url('salesorder/list_salesorder/' . $_inv->id); ?>" target="_blank"
...
<?php echo form_hidden('_attachment_sale_type', 'salesorder'); ?>
            ...
            <div class="horizontal-scrollable-tabs preview-tabs-top panel-full-width-tabs">
                ...
                <div class="horizontal-tabs">
                    <ul class="nav nav-tabs nav-tabs-horizontal mbot15" role="tablist">
						...
						<li role="presentation">
                            <a href="#tab_tasks"
                                onclick="init_rel_tasks_table(<?php echo $invoice->id; ?>,'salesorder'); return false;"
    ...
    [
            ...
            <div class="row mtop20">
                <div class="col-md-4">
					...
					<?php if(isset($create_pr) && $create_pr==1){//if(($check_create_purchase_request && $manufacturing_order->status != 'draft') || (!$pur_order_exist) ){ ?>
						<a href="<?php echo admin_url('salesorder/mo_create_purchase_request/' . $invoice->id); ?>"><button type="button" class="btn btn-success pull-left mo_create_purchase_request mright5" data-toggle="tooltip" title="" data-original-title="<?php echo _l('create_purchase_request_title'); ?>"><?php echo _l('mo_create_purchase_request'); ?> <i class="fa fa-question-circle i_tooltip" ></i></button></a>
					<?php } ?>
                </div>
                <div class="col-md-8 _buttons">
                    ...
                    <div class="pull-right">
                     ...
                     ?>
                        <?php if (has_permission('salesorder', '', 'edit')) { ?>
                        <a href="<?php echo admin_url('salesorder/invoice/' . $invoice->id); ?>" data-toggle="tooltip"
                            title="<?php echo _l('edit_salesorder_tooltip'); ?>" class="btn btn-default btn-with-tooltip"
                         ...
                         echo ' PDF';
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li class="hidden-xs"><a
                                        href="<?php echo admin_url('salesorder/pdf/' . $invoice->id . '?output_type=I'); ?>"><?php echo _l('view_pdf'); ?></a>
                                </li>
                                <li class="hidden-xs"><a
                                        href="<?php echo admin_url('salesorder/pdf/' . $invoice->id . '?output_type=I'); ?>"
                                        target="_blank"><?php echo _l('view_pdf_in_new_window'); ?></a></li>
                                <li><a
                                        href="<?php echo admin_url('salesorder/pdf/' . $invoice->id); ?>"><?php echo _l('download'); ?></a>
                                </li>
                                <li>
                                    <a href="<?php echo admin_url('salesorder/pdf/' . $invoice->id . '?print=true'); ?>"
                                        target="_blank">
                                        <?php echo _l('print'); ?>
                                    </a>
                                </li>
                        ...
                        <span<?php if ($invoice->status == Invoices_model::STATUS_CANCELLED) { ?> data-toggle="tooltip"
                         ...
                         echo ' disabled';
                           ...
                           && !empty($invoice->clientid)) {?>
                                    ...
                                    <?php if (has_permission('salesorder', '', 'create')) { ?>
                                    <li>
                                        <a
                                            href="<?php echo admin_url('salesorder/copy/' . $invoice->id); ?>"><?php echo _l('salesorder_copy'); ?></a>
                                    </li>
                                    <?php } ?>
									<li>
                                        <a
                                            href="<?php echo admin_url('salesorder/mark_as_cancelled/' . $invoice->id); ?>"><?php echo _l('salesorder_mark_as_closed'); ?></a>
                                    </li>
                                    ...
                                    <?php } ?>-->
                                    <!--<?php if (has_permission('salesorder', '', 'edit') || has_permission('salesorder', '', 'create')) { ?>
                                    <li>
                                        <?php if ($invoice->status != Invoices_model::STATUS_CANCELLED
                           ...
                           && has_permission('salesorder', '', 'edit')
                           && $invoice->duedate
                           && is_invoices_overdue_reminders_enabled()) { ?>
                                    ...
                                    <?php } ?>-->
                                    <?php
                           if ((get_option('delete_only_on_last_invoice') == 1 && is_last_salesorder($invoice->id)) || (get_option('delete_only_on_last_invoice') == 0)) { ?>
                                    <?php if (has_permission('salesorder', '', 'delete')) { ?>
                                    <li data-toggle="tooltip" data-title="<?php echo _l('delete_salesorder_tooltip'); ?>">
                                        <a href="<?php echo admin_url('salesorder/delete/' . $invoice->id); ?>"
							...
							<!--<div class="btn-group">
                                <button type="button" class="btn btn-default pull-left dropdown-toggle mleft10 pull-right btn btn-success"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    <i class="fa fa-plus-square"></i> <?php echo _l('convert'); ?> 
                                </button>
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li>
                                        <a href="#" data-toggle="modal"
                                            data-target="#sales_attach_file"><?php echo _l('salesorder_convert_to_invoice'); ?></a>
                                    </li>
                                    <li>
                                        <a
                                            href="<?php echo admin_url('salesorder/copy/' . $invoice->id); ?>"><?php echo _l('salesorder_convert_to_dc'); ?></a>
                                    </li>
                                    <?php hooks()->do_action('after_invoice_preview_more_menu'); ?>
                                </ul>
                            </div>-->
                            ...
                            <ul class="dropdown-menu">
                                <li>
									<a href="<?php echo admin_url('invoices/so_to_invoice/' . $invoice->id); ?>" <?php if($invoice->status=="Fully Delivered"){ ?> style="cursor:not-allowed;" onclick="return false;" <?php } ?> ><?php echo _l('salesorder_convert_to_invoice'); ?></a>
									<!--<a href="#" data-template="invoice" onclick="salesorder_convert_template(this,<?php echo $invoice->id; ?>); return false;"><?php echo _l('salesorder_convert_to_invoice'); ?></a>-->
                                </li>
                                <li class="divider">
                                <li><a
                                        href="<?php echo admin_url('warehouse/so_to_dc/' . $invoice->id); ?>" <?php if($invoice->status=="Fully Delivered"){ ?> style="cursor:not-allowed;" onclick="return false;" <?php } ?> ><?php echo _l('salesorder_convert_to_dc'); ?></a>
                                </li>
                                </li>
                            </ul>
                        </div>
            ...
            <div class="tab-content">
                <div role="tabpanel" class="tab-pane active" id="tab_invoice">
                    ...
                    <?php } ?>
                    <?php $this->load->view('admin/salesorder/invoice_preview_html'); ?>
                </div>
                <?php if (count($invoice->payments) > 0) { ?>
                ...
                <div role="tabpanel" class="tab-pane" id="tab_tasks">
                    <?php init_relation_tasks_table(['data-new-rel-id' => $invoice->id, 'data-new-rel-type' => 'salesorder']); ?>
                </div>
...
/*$('.mo_create_purchase_request').on('click', function() {
	...
	//alert(admin_url + 'manufacturing/mo_create_purchase_request/' + id);
	$.get(admin_url + 'salesorder/mo_create_purchase_request/' + id, function (response) {
