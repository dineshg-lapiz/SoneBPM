    ...
    <div class="alert alert-info">
        <div class="row">
            <div id="merge" class="col-md-6">
                <?php
              if (isset($invoice)) {
                  $this->load->view('admin/salesorder/merge_invoice', ['invoices_to_merge' => $invoices_to_merge]);
              }
            ?>
            </div>
            ...
            <div id="expenses_to_bill" class="col-md-6">
                ...
                $this->load->view('admin/salesorder/bill_expenses', ['expenses_to_bill' => $expenses_to_bill]);
            } ?>
            </div>
            <?php } ?>
        </div>
    ...
    <div class="panel-body">
        ...
        <div class="row">
            ...
            if (!isset($invoice_from_project)) { ?>
                   ...
                   ?>
					...
					<div class="col-md-6">
						<label for="proposal_template"
							class="control-label"><?php echo _l('template'); ?></label>
						<select name="template_id" class="selectpicker" data-width="100%"
							data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
							<option value="" selected>None</option>
							<?php foreach($templates as $template) { ?>
								<option value="<?php echo $template["id"]; ?>" <?php if (isset($invoice) && $invoice->template_id == $template["id"]) { echo 'selected'; } ?>><?php echo $template["name"]; ?></option>
							<?php } ?>
						</select>
					</div>
                </div>
               ...
               ?>
                <?php echo render_custom_fields('salesorder', $rel_id); ?>
            </div>
            <div class="col-md-6">
                <div class="tw-ml-3">
                    <div class="form-group">
                ...
                <thead>
                  ...
                  foreach ($custom_fields as $cf) {
                     ...
                     ?>
						...
						<th width="5%" align="right"><?php echo _l('discount'); ?></th>
	                    <th width="5%" align="right"><?php echo _l('discount(money)'); ?></th>
						...
						<td>
							<div class="input-group" id="discount-total">
								...
								<input type="number"
									...
									class="form-control pull-left item-input-discount-percent<?php if (isset($estimate) && !is_sale_discount($estimate, 'percent') && is_sale_discount_applied($estimate)) {
			   echo ' hide';
		   } ?>" min="0" max="100" name="item_discount_percent">
		
										
							</div>
						</td>
						<td class="item-discount-total"><input type="text" name="item_discount_amount" readonly="readonly" class="form-control"
								placeholder="<?php echo _l('Amount'); ?>"></td>
                        <td></td>
                        <td>
                            <?php
                        $new_item = 'undefined';
                    ...
                    <?php if (isset($invoice) || isset($add_items)) {
                            ...
                            foreach ($add_items as $item) {
								...
								$table_row .= '<td><div class="input-group" id="discount-total"><input type="number" data-item_discount_percent value="' . $item['discount_percent'] . '" onchange="calculate_total();" onblur="calculate_total();" class="form-control pull-left item-input-discount-percent" min="0" name="' . $items_indicator . '[' . $i . '][item_discount_percent]"></div></td>';
								 $table_row .= '<td class="item-discount-total"><input type="text" data-item_discount_amount name="' . $items_indicator . '[' . $i . '][item_discount_amount]" readonly="readonly" class="form-control" placeholder="Amount" value="' . $item['discount_amount'] . '"></td>';
