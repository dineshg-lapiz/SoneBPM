...
<div id="invoice-preview">
    <div class="row">
      ...
      if ($invoice->recurring > 0 || $invoice->is_recurring_from != null) {
        ...
        ?>
                <a href="<?php echo admin_url('salesorder/invoice/' . $invoice->id); ?>">
                    <span id="invoice-number">
                        <?php echo format_salesorder_number($invoice->id); ?>
                    </span>
                </a>
            </h4>
            <address>
			...
			$result = $CI->salesorder_model->get_purchase_request($invoice->id);
			/*print "<pre>";print_r($result);
			print $this->session->flashdata('reference_purchase_request'); */
			if(isset($result->pur_rq_code) && $result->pur_rq_code != ''){ ?>
				
					<span class="bold"><?php echo _l('reference_purchase_request'); ?> : </span>
					<span><a href="<?php echo admin_url('purchase/view_pur_request/'.$result->id) ?>" ><?php echo ($result->pur_rq_code) ?></a></span>
        ...
        <div class="col-md-5 col-md-offset-7">
            <table class="table text-right">
              ...
              ?>
                    ...
                    <?php if (count($invoice->payments) > 0 && get_option('show_total_paid_on_salesorder') == 1) { ?>
                    <tr>
                        <td>
                    ...
                    <?php } ?>
                    <?php if (get_option('show_amount_due_on_salesorder') == 1 && $invoice->status != Invoices_model::STATUS_CANCELLED) { ?>
                    <tr>
                        <td>
    ...
    <div class="mbot15 row inline-block full-width" data-attachment-id="<?php echo $attachment['id']; ?>">
        ...
        <div class="col-md-4 text-right tw-space-x-2">
            ...
            <a href="#" class="text-danger"
                onclick="delete_salesorder_attachment(<?php echo $attachment['id']; ?>); return false;"><i
