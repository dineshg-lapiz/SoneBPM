...
<script>
 ...
 var report_salesorder = $('#salesorder-reports');	
 var report_log = $('#log-reports');	
 var dc_report_log = $('#dc-log-reports');
 ...
 $(function() {
   ...
   $('.table-salesorder-report').on('draw.dt', function() {	
     var invoiceReportsTable = $(this).DataTable();	
     var sums = invoiceReportsTable.ajax.json().sums;	
     add_common_footer_sums($(this),sums);	
     $(this).find('tfoot td.amount_open').html(sums.amount_open);	
   ...
   $('.table-log-report').on('draw.dt', function() {	
     var visitlogReportsTable = $(this).DataTable();	
     /*var sums = visitlogReportsTable.ajax.json().sums;	
     add_common_footer_sums($(this),sums);	
     $(this).find('tfoot td.amount_open').html(sums.amount_open);	
     $(this).find('tfoot td.applied_credits').html(sums.applied_credits);	
     <?php foreach ($invoice_taxes as $key => $tax) { ?>	
 ...
 function init_report(e, type) {
   ...
   report_salesorder.addClass('hide');	
   report_log.addClass('hide');	
   dc_report_log.addClass('hide');
   ...
   // Clear custom date picker
       ...
       else if (type == 'salesorder-report') { 	
         report_salesorder.removeClass('hide');	
       }	
	   else if (type == 'log-report') { 	
         report_log.removeClass('hide');	
       }	
	   else if (type == 'dc-log-report') { 	
         dc_report_log.removeClass('hide');	
       }
   ...
   function salesorder_report() {	
     if ($.fn.DataTable.isDataTable('.table-salesorder-report')) {	
       $('.table-salesorder-report').DataTable().destroy();	
     }	
     initDataTable('.table-salesorder-report', admin_url + 'reports/salesorder_report', false, false, fnServerParams, [1, 'desc']);	
   }	
   function log_report() { 	
     if ($.fn.DataTable.isDataTable('.table-log-report')) {	
       $('.table-log-report').DataTable().destroy();	
     }	
	 //alert(admin_url + 'reports/visitlogs_report');	
     initDataTable('.table-log-report', admin_url + 'reports/visitlogs_report', false, false, fnServerParams, [1, 'desc']);	
   }	
   	
   function dc_log_report() { 	
     if ($.fn.DataTable.isDataTable('.table-dc-log-report')) {	
       $('.table-dc-log-report').DataTable().destroy();	
     }	
	 //alert(admin_url + 'reports/dc_visitlogs_report');	
     initDataTable('.table-dc-log-report', admin_url + 'reports/dc_visitlogs_report', false, false, fnServerParams, [1, 'desc']);	
   }
 ...
 function items_report(){
   ...
   function gen_reports() {
     ...
     if (!$('.chart-income').hasClass('hide')) {
       ...
       invoices_report();
     } else if (!report_salesorder.hasClass('hide')) {	
       salesorder_report();	
     } 	
	 else if (!report_log.hasClass('hide')) {	
       log_report();	
     }	
	 else if (!dc_report_log.hasClass('hide')) {	
       dc_log_report();	
