...
function app_init_admin_sidebar_menu_items()
{
	...
	if (is_admin()) {
    	$CI->app_menu->add_sidebar_menu_item('templates', [
            'name'     => _l('als_templates'),
            'href'     => admin_url('template_list/templates'),
            'position' => 1,
            'badge'    => [],
        ]);
	}
	...
	if((has_permission('salesorder', '', 'view') || has_permission('salesorder', '', 'view_own'))
        || (staff_has_assigned_salesorder() && get_option('allow_staff_view_salesorder_assigned') == 1)
    ){
        $CI->app_menu->add_sidebar_children_item('sales', [
            'slug'     => 'salesorder',
            'name'     => _l('salesorder'),
    ...
    if ((has_permission('Proforma_Invoice', '', 'view') || has_permission('Proforma_Invoice', '', 'view_own'))	
        || (staff_has_assigned_proposals() && get_option('allow_staff_view_proposals_assigned') == 1)	
    ) {	
        $CI->app_menu->add_sidebar_children_item('sales', [	
            'slug'     => 'Proforma_Invoice',	
            'name'     => _l('Proforma Invoice'),	
            'href'     => admin_url('Proforma_Invoice'),	
            'position' => 16,	
            'badge'    => [],	
        ]);	
    }
