...
function add_new_sales_item_post($item, $rel_id, $rel_type)
{
    ...
    $CI->db->insert(db_prefix() . 'itemable', [
                    ...
                    'unit'             => $item['unit'],
					'discount_percent' => $item['item_discount_percent'],
...
function update_sales_item_post($item_id, $data, $field = '')
{
    ...
    if ($field !== '') {
        if ($field == 'long_description') {
            ...
            $update[$field] = $data['item_discount_percent'];
        } elseif ($field == 'discount_amount') {
            $update[$field] = $data['item_discount_amount'];
        } else {
        ...
        $update = [
            ...
            'qty'              => $data['qty'],
            'unit'             => $data['unit'],
			'discount_percent' => $data['item_discount_percent'],
			'discount_amount' => $data['item_discount_amount'],
        ];
    }
...
function handle_removed_sales_item_post_salesorder($id, $rel_type)
{
    $CI = &get_instance();
