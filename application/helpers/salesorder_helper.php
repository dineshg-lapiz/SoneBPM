 ...
 * @return string Url
 */
function get_salesorder_shortlink($invoice)
{
    ...
    if ($short_link) {
        ...
        $CI->db->update(db_prefix() . 'salesorder', [
 ...
 */
function get_salesorder_total_left_to_pay($id, $invoice_total = null)
{
    $CI = &get_instance();
    ...
    if ($invoice_total === null) {
        $CI->db->select('total')
            ->where('id', $id);
        $invoice_total = $CI->db->get(db_prefix() . 'salesorder')->row()->total;
    }
 ...
 * @return boolean
 */
function is_salesorder_email_overdue_notice_enabled()
{
 ...
 * @return boolean
 */
function is_salesorder_email_due_notice_enabled()
{
 ...
 * @return boolean
 */
function is_salesorder_overdue_reminders_enabled()
{
    return is_salesorder_email_overdue_notice_enabled() || is_sms_trigger_active(SMS_TRIGGER_INVOICE_OVERDUE);
}
 ...
 * @return boolean
 */
function is_salesorder_due_reminders_enabled()
{
    return is_salesorder_email_due_notice_enabled() || is_sms_trigger_active(SMS_TRIGGER_INVOICE_DUE);
}
 ...
 */
function check_salesorder_restrictions($id, $hash)
{
    $CI = &get_instance();
 ...
 * @return mixed
 */
function format_salesorder_status($status, $classes = '', $label = true)
{
 ...
 * @return string
 */
function get_salesorder_status_label($status)
{
    if (!class_exists('Invoices_model', false)) {
        get_instance()->load->model('salesorder_model');
    }
 ...
 * @return boolean
 */
function is_salesorder_overdue($invoice)
{
    if (!class_exists('Invoices_model', false)) {
        get_instance()->load->model('salesorder_model');
    }
 ...
 * @return string
 */
function salesorder_status_color_pdf($status_id)
{
    $statusColor = '';
 ...
 */
function update_salesorder_status($id, $force_update = false, $prevent_logging = false)
{
    $CI = &get_instance();
    ...
    if (($original_status == Salesorder_model::STATUS_DRAFT && $force_update == false)
        || ($original_status == Salesorder_model::STATUS_CANCELLED && $force_update == false)
    ) {
        return false;
    }
            ...
            if ((function_exists('bccomp')
                ...
                if ($invoice->duedate != null) {
                    if ($totalPayments > 0) {
                        ...
                        $status = Salesorder_model::STATUS_PARTIALLY;
                    } elseif (date('Y-m-d', strtotime($invoice->duedate)) < date('Y-m-d')) {
                        $status = Salesorder_model::STATUS_OVERDUE;
                    }
                } else {
                    // Not paid completely status
                    $status = Salesorder_model::STATUS_PARTIALLY;
    ...
    if ($CI->db->affected_rows() > 0) {
        ...
        $CI->salesorder_model->log_invoice_activity($invoice->id, 'invoice_activity_status_updated', false, $additional_activity);
 ...
 * @return boolean
 */
function is_last_salesorder($id)
{
    $CI = &get_instance();
...
function format_invoice_number($id)
{
    ...
    if (! is_object($id)) {
        $CI->db->select('date,number,prefix,number_format')->from(db_prefix() . 'salesorder')->where('id', $id);
        $invoice = $CI->db->get()->row();
    } else {
        $invoice = $id;
        $id       = $invoice->id;
 ...
 * @return array
 */
function get_salesorder_item_taxes($itemid)
{
    $CI = &get_instance();
 ...
 * @return boolean
 */
function is_payment_mode_allowed_for_salesorder($id, $invoiceid)
{
    $CI = &get_instance();
    $CI->db->select('' . db_prefix() . 'currencies.name as currency_name,allowed_payment_modes')->from(db_prefix() . 'salesorder')->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'invoices.currency', 'left')->where(db_prefix() . 'invoices.id', $invoiceid);
    $invoice       = $CI->db->get()->row();
 ...
 * @return boolean
 */
function found_salesorder_mode($modes, $invoiceid, $offline = true, $show_on_pdf = false)
{
    $CI = &get_instance();
    $CI->db->select('' . db_prefix() . 'currencies.name as currency_name,allowed_payment_modes')->from(db_prefix() . 'salesorder')->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'invoices.currency', 'left')->where(db_prefix() . 'invoices.id', $invoiceid);
    $invoice = $CI->db->get()->row();
 ...
 * @return array
 */
function get_salesorder_percent_by_status($status)
{
    $has_permission_view = has_permission('invoices', '', 'view');
    $total_invoices      = total_rows(db_prefix() . 'salesorder', 'status NOT IN(5)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));

    $data            = [];
    $total_by_status = 0;
    if (!is_numeric($status)) {
        if ($status == 'not_sent') {
            $total_by_status = total_rows(db_prefix() . 'salesorder', 'sent=0 AND status NOT IN(2,5)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));
        }
    } else {
        $total_by_status = total_rows(db_prefix() . 'salesorder', 'status = ' . $status . ' AND status NOT IN(5)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));
    }
...
/**
 * Check if staff member have assigned salesorder/ added as sale agent
 * @param  mixed $staff_id staff id to check
 * @return boolean
 */
function staff_has_assigned_salesorder($staff_id = '')
{
    $CI       = &get_instance();
 ...
 * @return string
 */
function load_salesorder_total_template()
{
    $CI = &get_instance();
    $CI->load->model('salesorder_model');
    $_data = $CI->input->post();
...
function get_salesorder_where_sql_for_staff($staff_id)
{
    $CI                                 = &get_instance();
 ...
 * @return boolean
 */
function user_can_view_salesorder($id, $staff_id = false)
{
    $CI = &get_instance();
    ...
    $CI->db->from(db_prefix() . 'salesorder');
    $CI->db->where('id', $id);
    $invoice = $CI->db->get()->row();
