...
function get_proforma_shortlink($invoice)

{

    $long_url = site_url("Proforma_Invoice/{$invoice->id}/{$invoice->hash}");
    ...
    $short_link = app_generate_short_link([
        ...
        'title'    => format_proforma_number($invoice->id),
    ...
    if ($short_link) {
        ...
        $CI->db->update(db_prefix() . 'invoicesproforma', [
...
function get_proforma_total_left_to_pay($id, $invoice_total = null)

{
    ...
    if ($invoice_total === null) {
        ...
        $invoice_total = $CI->db->get(db_prefix() . 'invoicesproforma')->row()->total;
...
function is_proforma_email_overdue_notice_enabled()
...
function is_proforma_email_due_notice_enabled()
...
function is_proforma_overdue_reminders_enabled()
...
function is_proforma_due_reminders_enabled()
...
function check_salesorder_restrictions_proforma($id, $hash)

{
    ...
    $invoice = $CI->salesorder_model->get($id);
...
function format_salesorder_status_proforma($status, $classes = '', $label = true)

{

    if (!class_exists('Salesorder_model', false)) {

        get_instance()->load->model('salesorder_model');
...
function get_salesorder_status_label_proforma($status)
...
function is_proforma_overdue($invoice)
...

{
    ...
    if ($CI->db->affected_rows() > 0) {
        ...
        $log = 'Invoice Status Updated [Invoice Number: ' . format_proforma_number($invoice->id) . ', From: ' . format_invoice_status($original_status, '', false) . ' To: ' . format_invoice_status($status, '', false) . ']';
...

{
    ...
    $CI->db->select('id')->from(db_prefix() . 'invoicesproforma')->order_by('id', 'desc')->limit(1);
...
function is_last_salesorder_proforma($id)

{

    $CI = &get_instance();

    $CI->db->select('id')->from(db_prefix() . 'salesorder')->order_by('id', 'desc')->limit(1);
...

{
    ...
    if (!is_object($id)) {
        ...
        $CI->db->select('date,number,prefix,number_format,status')
            ...
            ->from(db_prefix() . 'invoicesproforma')
...
function format_salesorder_number_proforma($id)

{
    ...
    if ($invoice->status == Salesorder_model::STATUS_DRAFT) {
    ...
    return hooks()->apply_filters('format_salesorder_number', $number, [
...
function is_payment_mode_allowed_for_proforma($id, $invoiceid)

{

    $CI = &get_instance();

    $CI->db->select('' . db_prefix() . 'currencies.name as currency_name,allowed_payment_modes')->from(db_prefix() . 'invoicesproforma')->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'invoicesproforma.currency', 'left')->where(db_prefix() . 'invoicesproforma.id', $invoiceid);
...
function found_proforma_mode($modes, $invoiceid, $offline = true, $show_on_pdf = false)

{

    $CI = &get_instance();

    $CI->db->select('' . db_prefix() . 'currencies.name as currency_name,allowed_payment_modes')->from(db_prefix() . 'invoicesproforma')->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'invoicesproforma.currency', 'left')->where(db_prefix() . 'invoices.id', $invoiceid);
...
function get_proforma_percent_by_status($status)

{

    $has_permission_view = has_permission('invoices', '', 'view');

    $total_invoices      = total_rows(db_prefix() . 'invoicesproforma', 'status NOT IN(5)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));
    ...
    if (!is_numeric($status)) {
        ...
        if ($status == 'not_sent') {
            ...
            $total_by_status = total_rows(db_prefix() . 'invoicesproforma', 'sent=0 AND status NOT IN(2,5)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));

        }

    } else {

        $total_by_status = total_rows(db_prefix() . 'invoicesproforma', 'status = ' . $status . ' AND status NOT IN(5)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));
...
function staff_has_assigned_proforma($staff_id = '')

{
    ...
    if (is_numeric($cache)) {
        ...
        $result = total_rows(db_prefix() . 'invoicesproforma', ['sale_agent' => $staff_id]);
...
function staff_has_assigned_salesorder_proforma($staff_id = '')

{
    ...
    $cache    = $CI->app_object_cache->get('staff-total-assigned-salesorder-' . $staff_id);
    ...
    if (is_numeric($cache)) {
        ...
        $result = total_rows(db_prefix() . 'salesorder', ['sale_agent' => $staff_id]);

        $CI->app_object_cache->add('staff-total-assigned-salesorder-' . $staff_id, $result);
...
function load_proforma_total_template()

{
    ...
    $CI->load->view('admin/Proforma_Invoice/invoices_total_template', $data);

}

function load_salesorder_total_template_proforma()

{
    ...
    $data['invoices_years'] = $CI->salesorder_model->get_invoices_years();
    ...
    $data['total_result'] = $CI->salesorder_model->get_invoices_total($_data);

    $data['_currency']    = $data['total_result']['currencyid'];



    $CI->load->view('admin/salesorder/invoices_total_template', $data);

}



function get_proforma_where_sql_for_staff($staff_id)

{
    ...
    if ($has_permission_view_own) {
        ...
        $whereUser = '((' . db_prefix() . 'invoicesproforma.addedfrom=' . $CI->db->escape_str($staff_id) . ' AND ' . db_prefix() . 'invoicesproforma.addedfrom IN (SELECT staff_id FROM ' . db_prefix() . 'staff_permissions WHERE feature = "invoices" AND capability="view_own"))';
...
function get_salesorder_where_sql_for_staff_proforma($staff_id)

{
    ...
    $allow_staff_view_invoices_assigned = get_option('allow_staff_view_salesorder_assigned');

    $whereUser                          = '';

    if ($has_permission_view_own) {

        $whereUser = '((' . db_prefix() . 'salesorder.addedfrom=' . $CI->db->escape_str($staff_id) . ' AND ' . db_prefix() . 'salesorder.addedfrom IN (SELECT staff_id FROM ' . db_prefix() . 'staff_permissions WHERE feature = "invoices" AND capability="view_own"))';
...

{
    ...
    $CI->db->from(db_prefix() . 'invoicesproforma');
