...
function format_salesorder_number($id)
{
    $CI = &get_instance();
...
/*function get_salesorder_where_sql_for_staff($staff_id)
{
    $CI                                 = &get_instance();
    $has_permission_view_own            = has_permission('salesorder', '', 'view_own');
	$has_permission_view_members        = has_permission('salesorder', '', 'view_members');
	...
	$staff_members = $CI->staff_model->get_staff_members($staff_id);    
	
    $allow_staff_view_invoices_assigned = get_option('allow_staff_view_salesorder_assigned');
    $whereUser                          = '';
    if ($has_permission_view_own) {
        $whereUser = '((' . db_prefix() . 'salesorder.addedfrom=' . $CI->db->escape_str($staff_id) . ' AND ' . db_prefix() . 'salesorder.addedfrom IN (SELECT staff_id FROM ' . db_prefix() . 'staff_permissions WHERE feature = "salesorder" AND capability="view_own"))';
        if ($allow_staff_view_invoices_assigned == 1) {
            $whereUser .= ' OR sale_agent=' . $CI->db->escape_str($staff_id);
        }
        $whereUser .= ')';
	...
	//if ($has_permission_view_members) {
	//	foreach ($staff_members as $member) {
	//		$whereUser .= ' OR (' . db_prefix() . 'salesorder.addedfrom=' . $member['staffid'] . ')';
	//	}
	//}
	$staff = $CI->staff_model->get($staff_id);
	//print "<pre>";print_r($staff);
	if(isset($staff_members)){
    	foreach ($staff_members as $member) { //print $member['role'];
    		$role         = (array)$CI->roles_model->get($staff->role);
    		//print "<pre>";print_r($role);exit;
    		if(isset($role) && in_array("view_members",$role["permissions"]["salesorder"])){
    			$whereUser .= ' OR (' . db_prefix() . 'salesorder.addedfrom=' . $member['staffid'] . ')';
    		}
    		if ($has_permission_view_members) {
    			$whereUser .= ' OR (' . db_prefix() . 'salesorder.addedfrom=' . $member['staffid'] . ')';
