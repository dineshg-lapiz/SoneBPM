...
//print "<pre>";print_r($data);exit;
    ...
    */
    ...
    public function remove_document($type,$clientid)
    {
        $this->db->where('userid', $clientid);
        $this->db->update(db_prefix() . 'clients', [$type => '']);
        return true;
    }
	public function add_checkin_data($data){
		...
		$this->db->insert(db_prefix() . 'client_se_logs', array_merge($data, [
            'datecreated' => date('Y-m-d H:i:s'),
            'addedfrom'   => is_staff_logged_in() ? get_staff_user_id() : 0,
	...
	public function verify_checked_id($client_id){
		$this->db->select('*');
        $this->db->from(db_prefix() . 'client_se_logs');
	...
	public function update_checkout_data($data){
		...
		$checked_in = $this->verify_checked_id($data["client_id"]);
        ...
        $this->db->update(db_prefix() . 'client_se_logs', $data);
