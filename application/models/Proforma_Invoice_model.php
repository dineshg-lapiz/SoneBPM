...
class Proforma_Invoice_model extends App_Model

{
...

    {
        ...
        return $this->db->query('SELECT DISTINCT(sale_agent) as sale_agent, CONCAT(firstname, \' \', lastname) as full_name FROM ' . db_prefix() . 'invoicesproforma JOIN ' . db_prefix() . 'staff ON ' . db_prefix() . 'staff.staffid=' . db_prefix() . 'invoicesproforma.sale_agent WHERE sale_agent != 0')->result_array();
 ...
 public function get_invoices_years()
    ...
    {
//invoices
     return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' . db_prefix() . 'invoicesproforma ORDER BY year DESC')->result_array();

    }

    public function get_unpaid_Proforma_Invoice()

    {

        if (!staff_can('view', 'Proforma_Invoice')) {

            $where = get_Proforma_Invoice_where_sql_for_staff(get_staff_user_id());

            $this->db->where($where);

        }

        $this->db->select(db_prefix() . 'Proforma_Invoice.*,' . get_sql_select_client_company());

        $this->db->join(db_prefix() . 'clients', db_prefix() . 'Proforma_Invoice.clientid=' . db_prefix() . 'clients.userid', 'left');
        ...
        $Proforma_Invoice = $this->db->get(db_prefix() . 'Proforma_Invoice')->result();
...

    {
        ...
        $this->db->select('*, ' . db_prefix() . 'currencies.id as currencyid, ' . db_prefix() . 'invoicesproforma.id as id, ' . db_prefix() . 'currencies.name as currency_name');

        $this->db->from(db_prefix() . 'invoicesproforma');

        $this->db->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'invoicesproforma.currency', 'left');

        $this->db->where($where);

        if (is_numeric($id)) {

            $this->db->where(db_prefix() . 'invoicesproforma' . '.id', $id);
...

    {
        ...
        $this->db->update(db_prefix() . 'Proforma_Invoice', [
     ...
     * Get this invoice generated recurring Proforma_Invoice
    ...
    public function get_invoice_recurring_Proforma_Invoice($id)

    {
        ...
        $Proforma_Invoice           = $this->db->get(db_prefix() . 'invoicesproforma')->result_array();

        $recurring_Proforma_Invoice = [];



        foreach ($Proforma_Invoice as $invoice) {

            $recurring_Proforma_Invoice[] = $this->get($invoice['id']);

        }



        return $recurring_Proforma_Invoice;
    ...
    public function get_Proforma_Invoice_total($data)

    {
        ...
        $has_permission_view                = has_permission('Proforma_Invoice', '', 'view');

        $has_permission_view_own            = has_permission('Proforma_Invoice', '', 'view_own');

        $allow_staff_view_Proforma_Invoice_assigned = get_option('allow_staff_view_Proforma_Invoice_assigned');

        $noPermissionsQuery                 = get_Proforma_Invoice_where_sql_for_staff(get_staff_user_id());
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            if ($i == 1) {
                ...
                $select .= ', (SELECT total - (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'invoicepaymentrecords WHERE invoiceid = ' . db_prefix() . 'Proforma_Invoice.id) - (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'credits WHERE ' . db_prefix() . 'credits.invoice_id=' . db_prefix() . 'Proforma_Invoice.id)) as outstanding';

            } elseif ($i == 2) {

                $select .= ',(SELECT SUM(amount) FROM ' . db_prefix() . 'invoicepaymentrecords WHERE invoiceid=' . db_prefix() . 'Proforma_Invoice.id) as total_paid';
            ...
            // Exclude cancelled Proforma_Invoice
            ...
            $Proforma_Invoice = $this->db->get()->result_array();
    ...
    {
//print_r($data); exit();
        $data['prefix'] = get_option('Proforma_invoice_prefix'); //invoice_prefix
        ...
        $Proforma_Invoice_to_merge = isset($data['Proforma_Invoice_to_merge']) ? $data['Proforma_Invoice_to_merge'] : [];



        $cancel_merged_Proforma_Invoice = isset($data['cancel_merged_Proforma_Invoice']);
...
//print "<pre>";print_r($data); print_r($items);print "</pre>";

//$sql = $this->db->set($data)->get_compiled_insert(db_prefix() . 'Proforma_Invoice');
...
//exit;
        ...
        $this->db->insert(db_prefix().'invoicesproforma', $data);//db_prefix().
...
//echo 'Test - 1';exit();
            ...
            foreach ($Proforma_Invoice_to_merge as $m) {

                $merged   = false;

                $or_merge = $this->get($m);

                if ($cancel_merged_Proforma_Invoice == false) {
                    ...
                    if ($this->mark_as_cancelled($m)) {
                        ...
                        $this->db->update(db_prefix() . 'invoicesproforma', [
...
//exit();
            ...
            update_sales_total_tax_column($insert_id, 'invoice', db_prefix() . 'invoicesproforma');
...

    {
        ...
        if ($current_invoice != '') {
            ...
            $row = $this->db->get(db_prefix() . 'invoicesproforma')->row();
        ...
        $noPermissionsQuery  = get_salesorder_where_sql_for_staff_proforma(get_staff_user_id());//get_salesorder_where_sql_for_staff_proforma
        ...
        $Proforma_Invoice = hooks()->apply_filters('Proforma_Invoice_ids_available_for_merging', $Proforma_Invoice);
...

    {
        ...
        $id = $this->Proforma_Invoice_model->add($new_invoice_data);
        ...
        if ($id) {
            ...
            log_activity('Copied Invoice ' . format_proforma_number($_invoice->id));
	...
	public function add_new_invoice($data, $expense = false)

    {

        $data['prefix'] = get_option('Proforma_invoice_prefix');
...
echo $sql;exit;*/
        ...
        $this->db->insert(db_prefix() . 'Proforma_Invoice', $data);
        ...
        if ($insert_id) {
            ...
            foreach ($Proforma_Invoice_to_merge as $m) {
                ...
                if ($cancel_merged_Proforma_Invoice == false) {
                    ...
                    if ($this->mark_as_cancelled($m)) {
                        ...
                        $note       = 'Merged into invoice ' . format_proforma_number($insert_id);
            ...
            update_sales_total_tax_column($insert_id, 'invoice', db_prefix() . 'Proforma_Invoice');
...
//exit();
        ...
        $hookData = [
            ...
            'Proforma_Invoice_to_merge'      => $Proforma_Invoice_to_merge,

            'cancel_merged_Proforma_Invoice' => $cancel_merged_Proforma_Invoice,
        ...
        $Proforma_Invoice_to_merge = $hook['Proforma_Invoice_to_merge'];
        ...
        $this->db->where('id', $id)->update('invoicesproforma', $data);
            ...
            if (isset($data['data']) && $original_number != $data['number']) {
                ...
                $this->log_invoice_activity(
                    ...
                    serialize([
                        ...
                        format_proforma_number($original_invoice->id),
        ...
        if ($this->merge_Proforma_Invoice($Proforma_Invoice_to_merge, $id, $cancel_merged_Proforma_Invoice)) {
        ...
        if ($updated) {
            ...
            update_sales_total_tax_column($id, 'invoice', db_prefix() . 'invoicesproforma');
...
//print "<pre>";print_r($original_item);exit;
    ...
    protected function merge_Proforma_Invoice($Proforma_Invoice, $id, $cancel)

    {

        foreach ($Proforma_Invoice as $mergeId) {
            ...
            if ($cancel == false) {
                ...
                if ($this->mark_as_cancelled($mergeId)) {
                    ...
                    $note       = 'Merged into invoice ' . format_proforma_number($id);
                    ...
                    $this->db->where('id', $mergeId)->update('Proforma_Invoice', [
...

    {
        ...
        foreach ($items as $item) {
			...
			if (update_sales_item_post($item['itemid'], $item, 'discount_percent')) {
                ...
                $this->log_invoice_activity($id, 'invoice_estimate_activity_updated_discount_percent_item', false, serialize([
			...
			if (update_sales_item_post($item['itemid'], $item, 'discount_amount')) {
                ...
                $this->log_invoice_activity($id, 'invoice_estimate_activity_updated_discount_amount_item', false, serialize([
...

    {
        ...
        $this->db->delete(db_prefix() . 'Proforma_Invoice');
...

    {
        ...
        $invoice_number = format_proforma_number($invoice->id);
...

    {
        ...
        if (is_array($send_to) && count($send_to) > 0) {
            ...
            $this->db->update('Proforma_Invoice', ['number' => $originalNumber]);
...

    {
        ...
        return total_rows('invoicesproforma', ['id' => $id, 'status' => self::STATUS_DRAFT]) === 1;
     ...
     * Get the Proforma_Invoice years
    ...
    public function get_Proforma_Invoice_years()

    {

        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' . db_prefix() . 'Proforma_Invoice ORDER BY year DESC')->result_array();
...

    {
        ...
        $invoice = $this->db->get('Proforma_Invoice')->row();
        ...
        $this->db->update('Proforma_Invoice', ['number' => $newNumber]);
