...
class Staff_model extends App_Model
{
    ...
    public function get_staff_members($id, $with_name = false)	
    {	
        /*if ($with_name) {	
		...
		$this->db->select('firstname,lastname,email,staffid,role');	
        $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'staff_members.member_id');	
        $this->db->where('staff_id', $id);	
        return $this->db->get(db_prefix() . 'staff_members')->result_array();	
    }
    ...
    public function add($data)
    {
        ...
        if ($staffid) {
            ...
            if (isset($staff_members)) {	
				...
				$this->db->delete(db_prefix() . 'staff_members');	
				foreach ($staff_members as $staff_member) {	
											
					$this->db->insert(db_prefix() . 'staff_members', [	
                            'staff_id'   => $staffid,	
							'member_id' => $staff_member,	
                        ]);	
    ...
    public function update($data, $id)
    {
        ...
        if (isset($staff_members)) {	
			//$_pm['staff_members'] = $staff_members;	
			//$this->add_edit_members($_pm, $staffid);	
			//print "<pre>";print_r($staff_members);exit;	
			$this->db->where('staff_id', $id);	
			$this->db->delete(db_prefix() . 'staff_members');	
			foreach ($staff_members as $staff_member) {	
