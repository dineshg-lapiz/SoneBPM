...
class Salesorder_model extends App_Model
{
	...
	private $statuses = [
		'UnPaid','Open','Partially Delivered','Fully Delivered','Overdue'];
		/*self::STATUS_UNPAID,
        self::STATUS_FULLY,
        self::STATUS_PARTIALLY,
        self::STATUS_OVERDUE,
    ...
    {
        return $this->db->query('SELECT DISTINCT(sale_agent) as sale_agent, CONCAT(firstname, \' \', lastname) as full_name FROM ' . db_prefix() . 'salesorder JOIN ' . db_prefix() . 'staff ON ' . db_prefix() . 'staff.staffid=' . db_prefix() . 'salesorder.sale_agent WHERE sale_agent != 0')->result_array();
    }
	
	public function get_item_available_qty($commodity_no){
		//print 'SELECT inventory_number FROM ' . db_prefix() . 'inventory_manage WHERE commodity_id="'.$commodity_no.'" ';
		return $this->db->query('SELECT inventory_number FROM ' . db_prefix() . 'inventory_manage WHERE commodity_id="'.$commodity_no.'" ')->result_array();
		//print "<pre>";print_r(($array_stock));
		//return ($array_stock);
	}
	public function get_companyname_by_clientid($clientid){
		//print 'SELECT company FROM ' . db_prefix() . 'clients WHERE userid="'.$clientid.'" ';
		return $this->db->query('SELECT company FROM ' . db_prefix() . 'clients WHERE userid="'.$clientid.'" ')->row();
		//print "<pre>";print_r(($array_stock));
		//return ($array_stock);
	}
	public function get_item_available_qty_by_name($name){
		//print 'SELECT inventory_number FROM ' . db_prefix() . 'inventory_manage WHERE commodity_id="'.$commodity_no.'" ';
		return $this->db->query('SELECT im.inventory_number FROM ' . db_prefix() . 'inventory_manage as im, ' . db_prefix() . 'items as it WHERE it.description="'.$name.'" and it.id=im.commodity_id ')->result_array();
		//print "<pre>";print_r(($array_stock));
		//return ($array_stock);
	}
	public function get_item_id_by_name($name){
		//print 'SELECT inventory_number FROM ' . db_prefix() . 'inventory_manage WHERE commodity_id="'.$commodity_no.'" ';
		return $this->db->query('SELECT it.id,it.purchase_price FROM ' . db_prefix() . 'items as it WHERE it.description="'.$name.'" ')->result_array();
		//print "<pre>";print_r(($array_stock));
		//return ($array_stock);
	}
	...
	public function add_pur_request($data){
        ...
        $p_data['request_date'] = date('Y-m-d H:i:s');
...
//print "<pre>";print_r($data);//exit;
		...
		$p_data['from_currency'] = $data['currency'];
        $p_data['to_currency'] = $data['currency'];
        ...
        if(isset($data['send_to_vendors']) && count($data['send_to_vendors']) > 0){
            $p_data['send_to_vendors'] = implode(',', $data['send_to_vendors']);
        }

        $p_data['subtotal'] = reformat_currency_pur($data['subtotal'], $data['currency']);

        if(isset($data['total_mn'])){
            $p_data['total'] = reformat_currency_pur($data['total_mn'], $data['currency']);    
            unset($data['total_mn']);
        }

        $p_data['total_tax'] = $data['total'] - $data['subtotal'];
        ...
        //while($check_exist_number) {
          $p_data['number'] = $data['number'] + 1;
		  $p_data['currency'] = $data['currency'];
		  $p_data['currency_rate'] = '1.000000';
          $p_data['pur_rq_code'] =  $prefix.'-'.str_pad($data['number'],5,'0',STR_PAD_LEFT).'-'.date('M-Y'); //.'-'.$dpm_name;
		  $p_data['pur_rq_name'] =  'PUR create from Saleorder '.format_salesorder_number($data["id"]);
		  $p_data['rq_description'] = '';
		  $p_data['requester'] = 2;
		  $p_data['department'] = 2;
		  $p_data['sale_saleorder'] = $data["id"];
		  
		  //unset($data);
		  /*unset($data['sent']);
		  unset($data['datesend']);
		  unset($data['clientid']);
		  unset($data['deleted_customer_name']);
		  unset($data['prefix']);
		  
		  unset($data['number_format']);
		  ...
		  unset($data['invoiced_date']);
		  
		  unset($data['clientnote']);
		  unset($data['adminnote']);
		  unset($data['last_overdue_reminder']);
		  unset($data['last_due_reminder']);
		  unset($data['cancel_overdue_reminders']);
		  ...
		  unset($data['custom_recurring']);
		  unset($data['cycles']);
		  unset($data['total_cycles']);
		  unset($data['is_recurring_from']);
		  unset($data['last_recurring_date']);
		  unset($data['terms']);
        ...
        $p_data['hash'] = app_generate_hash();
      //print "<pre>";print_r($data);//exit;
	  $p_data = (object) $p_data;
	  //print "<pre>";print_r($p_data);//exit;
        $this->db->insert(db_prefix().'pur_request',$p_data);
...
//print "<pre>";print_r($detail_data);
            if(count($detail_data) > 0){
                foreach($detail_data as $key => $rqd){
					//print "<pre>";print_r($rqd);//exit;
					$item_id = $this->get_item_id_by_name($rqd["description"]);
					$available_qty = $this->get_item_available_qty_by_name($rqd["description"]);
					//print "<pre>";print_r($available_qty);
                    $dt_data = [];
                    $dt_data['pur_request'] = $insert_id;
                    $dt_data['item_code'] = $item_id[0]["id"];
                    $dt_data['unit_id'] = isset($rqd['unit_id']) ? $rqd['unit_id'] : null;
                    $dt_data['unit_price'] = $item_id[0]["purchase_price"];//$rqd['rate'];
                    
                    //$dt_data['total'] = $data['total'];
                    //$dt_data['tax_value'] = $data['total_tax'];
                    $dt_data['item_text'] = $rqd['description'];
					$dt_data['quantity'] = $rqd["qty"]-$available_qty[0]["inventory_number"];//$rqd["qty"];//($rqd['quantity'] != ''&& $rqd['quantity'] != null) ? $rqd['quantity'] : 0;
                    $tax_money = 0;
                    $tax_rate_value = 0;
					$tax_value = 0;
                    $tax_rate = '';
                    ...
                    $tax_rate_data = get_invoice_item_taxes($rqd['id']);
					/*print "123";
					print "<pre>";print_r($tax_rate_data);*/
					
					if(isset($tax_rate_data) && isset($tax_rate_data[0]["id"])){
						$data_tax = [];
						foreach($tax_rate_data as $tax_data){
							$data_tax[] = $tax_data["taxname"];
						}
						$pur_data_tax = $this->pur_get_tax_rate($data_tax);
						//print "<pre>";print_r($pur_data_tax);
						$tax_rate_value = $pur_data_tax['tax_rate'];
						$tax_rate = $pur_data_tax['tax_rate_str'];
						$tax_id = $pur_data_tax['tax_id_str'];
						$tax_name = $pur_data_tax['tax_name_str'];
						foreach($tax_rate_data as $tax_data){
							$tax_value += (float)$dt_data['quantity'] * (float)$dt_data['unit_price'] * (float)$tax_rate / 100;
						}
					...
					$dt_data['tax_value'] = $tax_value;
					//$tax_value += $item['qty'] * $item['rate'] * $_tax['taxrate'] / 100;

                    
					$dt_data['into_money'] = $dt_data["quantity"]*$dt_data['unit_price'];
					
					
					
					$dt_data['total'] = $dt_data['into_money'] + $dt_data['tax_value']; //$data['total'];
//print "<pre>";print_r($dt_data);
	...
	public function get_purchase_request($id = ''){
            ...
            $this->db->where('sale_saleorder',$id);
    ...
    public function get_unpaid_invoices()
    {
        if (!staff_can('view', 'invoices')) {
            $where = get_salesorder_where_sql_for_staff(get_staff_user_id());
            $this->db->where($where);
        }
        $this->db->select(db_prefix() . 'salesorder.*,' . get_sql_select_client_company());
        $this->db->join(db_prefix() . 'clients', db_prefix() . 'salesorder.clientid=' . db_prefix() . 'clients.userid', 'left');
        $this->db->where_not_in('status', [self::STATUS_CANCELLED, self::STATUS_PAID]);
        $this->db->where('total >', 0);
        $this->db->order_by('number,YEAR(date)', 'desc');
        $invoices = $this->db->get(db_prefix() . 'salesorder')->result();
    ...
    { 
        $this->db->select('*, ' . db_prefix() . 'currencies.id as currencyid, ' . db_prefix() . 'salesorder.id as id, ' . db_prefix() . 'currencies.name as currency_name');
        $this->db->from(db_prefix() . 'salesorder');
        $this->db->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'salesorder.currency', 'left');
        $this->db->where($where);

        if (is_numeric($id)) { 
            $this->db->where(db_prefix() . 'salesorder' . '.id', $id);
            $invoice = $this->db->get()->row();
			
            if ($invoice) {
                $invoice->total_left_to_pay = get_invoice_total_left_to_pay($invoice->id, $invoice->total);

                $invoice->items       = get_items_by_type('salesorder', $id);
                ...
                $invoice->payments = array();//$this->payments_model->get_invoice_payments($id);
	...
	public function get_invoice_item_by_rel($id)
    {
	...
	public function get_saleorder_item_by_rel($id)
    {
        $this->db->where('rel_id', $id);
		$this->db->where('rel_type', 'salesorder');
    ...
    public function get_invoice_recurring_invoices($id)
    {
        ...
        $invoices           = $this->db->get(db_prefix() . 'salesorder')->result_array();
        $recurring_invoices = [];
    ...
    public function get_invoices_total($data)
    {
        ...
        $has_permission_view_own            = has_permission('salesorder', '', 'view_own');
        $allow_staff_view_invoices_assigned = get_option('allow_staff_view_invoices_assigned');
        $noPermissionsQuery                 = get_salesorder_where_sql_for_staff(get_staff_user_id());

        //for ($i = 1; $i <= 3; $i++) {
            $select = 'id,total';
            /*if ($i == 1) {
                $select .= ', (SELECT SUM(total) FROM ' . db_prefix() . 'salesorder WHERE (status="Open" or status="Partially Delivered")) as outstanding , (SELECT SUM(total) FROM ' . db_prefix() . 'salesorder WHERE (status="Partially Delivered")) as partiallydelivered, (SELECT SUM(total) FROM ' . db_prefix() . 'salesorder WHERE (status="Open")) as open';
            } elseif ($i == 2) {
                $select .= ',(SELECT SUM(amount) FROM ' . db_prefix() . 'invoicepaymentrecords WHERE invoiceid=' . db_prefix() . 'salesorder.id) as total_paid';
            }*/
			$select .= ', (SELECT SUM(total) FROM ' . db_prefix() . 'salesorder WHERE (status="Open" or status="Partially Delivered")) as outstanding , (SELECT SUM(total) FROM ' . db_prefix() . 'salesorder WHERE (status="Partially Delivered")) as partiallydelivered, (SELECT SUM(total) FROM ' . db_prefix() . 'salesorder WHERE (status="Open")) as open';
...
//print "<pre>";print_r($invoices);
            //foreach ($invoices as $invoice) {
				//print "<pre>";print_r($invoices);
				if(isset($invoices[0])){
					$result['due'] = $invoices[0]['outstanding'];
					$result['paid'] = $invoices[0]['partiallydelivered'];
					$result['overdue'] = $invoices[0]['open'];
				}else{
					$result['due'] = 0;
					$result['paid'] = 0;
					$result['overdue'] = 0;
                ...
                /*if ($i == 1) {
                    ...
                    $result['paid'][] = $invoice['partiallydelivered'];
                } elseif ($i == 3) {
                    $result['overdue'][] = $invoice['open'];
                }*/
            //}
        //}
        $currency             = get_currency($currencyid);
        $result['due']        = ($result['due']);
        $result['partiallydelivered']       = ($result['paid']);
        $result['open']    = ($result['overdue']);
...
//print "<pre>";print_r($result);
    ...
    {
        $data['prefix'] = get_option('salesorder_prefix');
		...
		//unset($data['item_discount_total']);
		unset($data['item_discount_amount']);
		//print "<pre>";print_r($data);//exit;
        $this->db->insert(db_prefix() . 'salesorder', $data);
        ...
        if ($insert_id) {
            ...
            handle_tags_save($tags, $insert_id, 'salesorder');
            ...
            foreach ($invoices_to_merge as $m) {
                ...
                if ($cancel_merged_invoices == false) {
                    ...
                    if ($this->mark_as_cancelled($m)) {
                        ...
                        $note       = 'Merged into invoice ' . format_salesorder_number($insert_id);
                        if ($admin_note != '') {
                            $admin_note .= "\n\r" . $note;
                        } else {
                            $admin_note = $note;
            ...
            foreach ($items as $key => $item) {
                if ($itemid = add_new_sales_item_post($item, $insert_id, 'salesorder')) {
            ...
            update_sales_total_tax_column($insert_id, 'invoice', db_prefix() . 'salesorder');

            if (!DEFINED('CRON') && $expense == false) {
                $lang_key = 'salesorder_activity_created';
            } elseif (!DEFINED('CRON') && $expense == true) {
                $lang_key = 'salesorder_activity_from_expense';
            } elseif (DEFINED('CRON') && $expense == false) {
                $lang_key = 'salesorder_activity_recurring_created';
            } else {
                $lang_key = 'salesorder_activity_recurring_from_expense_created';
            }
	...
	public function add_new_salesorder($data, $expense = false)
    {
        $data['prefix'] = get_option('salesorder_prefix');
...
//print "<pre>";print_r($hook);
        $data  = $hook['data'];
        $items = $hook['items'];
		$new_items         = $hook['new_items'];
        $removed_items     = $hook['removed_items'];
/*print "<pre>";print_r($items);
print "<pre>";print_r($new_items);
$sql = $this->db->set($data)->get_compiled_insert(db_prefix() . 'invoices');
echo $sql;exit;*/
        ...
        $this->db->insert(db_prefix() . 'salesorder', $data);
        $insert_id = $this->db->insert_id();
		/*if ($this->save_items($items, $insert_id)) {
			$updated = true;
		}*/
		
		if ($this->add_new_items($newitems, $billed_tasks, $billed_expenses, $insert_id)) {
			$updated = true;
		}
		
		if ($this->remove_items_salesorder($removed_items, $insert_id)) {
			$updated = true;
		}
	...
	protected function remove_items_salesorder($items, $id)
    {
        $updated = false;
		//print "<pre>";print_r($items);//exit;
        foreach ($items as $itemId) {
            $original_item = $this->get_invoice_item($itemId);
//print "<pre>";print_r($original_item);exit;
            if (handle_removed_sales_item_post_salesorder($itemId, 'invoice')) {
                $updated = true;
    ...
    public function check_for_merge_invoice($client_id, $current_invoice = '')
    {
        if ($current_invoice != '') {
            ...
            $row = $this->db->get(db_prefix() . 'salesorder')->row();
	...
	public function convert_to_dc($id){
		/*$salesorder = $this->get($id);
		$dc = [];*/
	}
	/**
    ...
    public function convert_to_invoice($id, $client = false, $draft_invoice = false)
    {
        ...
        $new_invoice_data['number']     = get_option('next_salesorder_number');
        ...
        $key                                       = 1;
		//print "<pre>";print_r($_estimate->items);
        foreach ($_estimate->items as $item) {
            $new_invoice_data['newitems'][$key]['description']      = $item['description'];
            $new_invoice_data['newitems'][$key]['long_description'] = clear_textarea_breaks($item['long_description']);
            $new_invoice_data['newitems'][$key]['qty']              = $item['qty'];
			$new_invoice_data['newitems'][$key]['available_quantity'] = $item['available_quantity'];
		...
		//print "<pre>";print_r($new_invoice_data['newitems']);exit;
        ...
        if ($id) {
            ...
            if (!is_staff_logged_in()) {
                ...
                $this->salesorder_model->log_invoice_activity($id, 'invoice_activity_auto_converted_from_salesorder', true, serialize([
                    '<a href="' . admin_url('salesorder/list_salesorder/' . $_estimate->id) . '">' . format_salesorder_number($_estimate->id) . '</a>',
                ]));
            }
            ...
            // Update salesorder with the new invoice data and set to status accepted
            ...
            if ($client == false) {
                $this->log_invoice_activity($_estimate->id, 'salesorder_activity_converted', false, serialize([
                    '<a href="' . admin_url('invoices/list_invoices/' . $id) . '">' . format_salesorder_number($id) . '</a>',
                ]));
            }
    ...
    public function copy($id)
    {
        ...
        $id = $this->salesorder_model->add($new_invoice_data);
        if ($id) {
            $this->db->where('id', $id);
            $this->db->update(db_prefix() . 'salesorder', [
            ...
            log_activity('Copied Invoice ' . format_salesorder_number($_invoice->id));
    ...
    public function update($data, $id)
    {
        ...
        $original_number_formatted = format_salesorder_number($id);
        ...
        $this->db->where('id', $id)->update('salesorder', $data);
            ...
            if (isset($data['data']) && $original_number != $data['number']) {
                $this->log_invoice_activity(
                    ...
                    serialize([
                        ...
                        format_salesorder_number($original_invoice->id),
                    ])
                );
            }
        }
        ...
        if ($updated) {
            update_sales_total_tax_column($id, 'invoice', db_prefix() . 'salesorder');
        ...
        foreach ($invoices as $mergeId) {
            ...
            if ($cancel == false) {
                ...
                if ($this->mark_as_cancelled($mergeId)) {
                    ...
                    $note       = 'Merged into invoice ' . format_salesorder_number($id);
                    if ($admin_note != '') {
                        $admin_note .= "\n\r" . $note;
                    } else {
                        $admin_note = $note;
    ...
    protected function add_new_items($items, $billed_tasks, $billed_expenses, $id)
    {
        ...
        foreach ($items as $key => $item) {
            if ($new_item_added = add_new_sales_item_post($item, $id, 'salesorder')) {
                if (isset($billed_tasks[$key])) {
                    foreach ($billed_tasks[$key] as $_task_id) {
    ...
    protected function save_items($items, $id)
    {
        ...
        foreach ($items as $item) {
			...
			/*if (update_sales_item_post($item['itemid'], $item, 'available_quantity')) {
                $this->log_invoice_activity($id, 'invoice_estimate_activity_updated_qty_item', false, serialize([
                    $item['description'],
                    $original_item->available_quantity,
                    $item['available_quantity'],
                ]));
                $updated = true;
            }*/
			
			if (update_sales_item_post($item['itemid'], $item, 'discount_percent')) {
                $this->log_estimate_activity($id, 'invoice_estimate_activity_updated_discount_percent_item', false, serialize([
                    $item['description'],
                    $original_item->discount_percent,
                    $item['item_discount_percent'],
                ]));
                $affectedRows++;
            }
			
			if (update_sales_item_post($item['itemid'], $item, 'discount_amount')) {
                $this->log_estimate_activity($id, 'invoice_estimate_activity_updated_discount_amount_item', false, serialize([
                    $item['description'],
                    $original_item->discount_amount,
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        $number  = format_salesorder_number($id);
        $isDraft = $this->is_draft($id);
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'salesorder');
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            get_option('next_salesorder_number') > 1 &&
            $simpleDelete == false &&
            !$isDraft) {
            ...
            $this->db->where('relid IN (SELECT id from ' . db_prefix() . 'itemable WHERE rel_type="salesorder" AND rel_id="' . $this->db->escape_str($id) . '")');
            $this->db->where('fieldto', 'items');
            $this->db->delete(db_prefix() . 'customfieldsvalues');

            $items = get_items_by_type('salesorder', $id);
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' . db_prefix() . 'salesorder ORDER BY year DESC')->result_array();
    }
    ...
    public function change_invoice_number_when_status_draft($id)
    {
        ...
        $invoice = $this->db->get('salesorder')->row();

        $newNumber = get_option('next_salesorder_number');

        $this->db->where('id', $id);
        $this->db->update('salesorder', ['number' => $newNumber]);
    ...
    public function increment_next_number()
    {
        ...
        $this->db->where('name', 'next_salesorder_number');
