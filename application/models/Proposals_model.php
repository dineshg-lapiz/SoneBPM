...
class Proposals_model extends App_Model
{
	...
	public function get_commodity_code_by_name($name){
		//print 'SELECT inventory_number FROM ' . db_prefix() . 'inventory_manage WHERE commodity_id="'.$commodity_no.'" ';
		return $this->db->query('SELECT it.id,it.purchase_price,it.commodity_code FROM ' . db_prefix() . 'items as it WHERE it.description="'.$name.'" ')->result_array();
