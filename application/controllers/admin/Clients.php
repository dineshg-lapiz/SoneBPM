...
class Clients extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['salesorder_statuses'] = $this->salesorder_model->get_statuses();
    ...
    public function client($id = '')
    {
        ...
        if ($id == '') {
            ...
            $data['checked_in'] = $this->clients_model->verify_checked_id($id);	
			$data['se_logs'] = $this->clients_model->se_logs($id);
        }
	...
	public function zip_salesorder($id)
    {
        $has_permission_view = has_permission('invoices', '', 'view');
        if (!$has_permission_view && !has_permission('invoices', '', 'view_own')
            && get_option('allow_staff_view_invoices_assigned') == '0') {
        ...
        if ($this->input->post()) {
            $this->load->library('app_bulk_pdf_export', [
                'export_type'       => 'salesorder',
                'status'            => $this->input->post('salesorder_zip_status'),
                'date_from'         => $this->input->post('zip-from'),
                'date_to'           => $this->input->post('zip-to'),
                'redirect_on_error' => admin_url('clients/client/' . $id . '?group=salesorder'),
            ]);
    ...
    {	
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {	
			...
			$data = $this->input->post();	
			$id = $this->clients_model->add_checkin_data($data);	
			if ($id) {	
				echo "success";	
				die;	
			}	
    ...
    {	
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {	
			//print "<pre>";print_r($this->input->post());exit;	
			$data = $this->input->post();	
			$id = $this->clients_model->update_checkout_data($data);	
			if ($id) {	
				echo "success";	
				die;	
			}	
