...
class Estimates extends AdminController
{
    ...
    public function estimate($id = '')
    {
		...
		$data['templates'] = $this->template_list_model->get_templates('estimate');
	...
	public function get_salesorder_convert_data($id)
    {
        ...
        $data['billable_tasks'] = [];
        $data['add_items']      = $this->_parse_items($data['estimate']);
        ...
        $this->load->view('admin/estimates/salesorder_convert_template', $data);
    }
	public function convert_to_salesorder($id)
    {
        ...
        if ($this->input->post()) {
            ...
            $invoice_id = $this->salesorder_model->add_new_salesorder($this->input->post());
            if ($invoice_id) {
                set_alert('success', _l('estimate_converted_to_salesorder_success'));
                $this->db->where('id', $id);
                $this->db->update(db_prefix() . 'estimates', [
                    'saleorder_id' => $invoice_id,
                    //'status'     => 3,
                ]);
                log_activity('Estimate Converted to Saleorder [SaleOrderID: ' . $invoice_id . ', EstimateID: ' . $id . ']');
                hooks()->do_action('estimate_converted_to_saleorder', ['estimate_id' => $id, 'saleorder_id' => $invoice_id]);
                redirect(admin_url('salesorder/list_salesorder/' . $invoice_id));
            } else {
                set_alert('danger', _l('estimate_converted_to_saleorder_fail'));
            }
			if ($this->set_estimate_pipeline_autoload($id)) {
				redirect(admin_url('estimates/list_estimates/'));
			} else {
    ...
    public function pdf($id)
    {
		...
		if($estimate->template_id!=""){
			$this->load->model('template_list_model');
			$estimate->templates = (array)$this->template_list_model->get($estimate->template_id);
		}
