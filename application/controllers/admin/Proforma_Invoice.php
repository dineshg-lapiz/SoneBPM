...
class Proforma_Invoice extends AdminController

{
...

    {
        ...
        if (!has_permission('Proforma_Invoice', '', 'view')

            && !has_permission('Proforma_Invoice', '', 'view_own')

            && get_option('allow_staff_view_Proforma_Invoice_assigned') == '0') {

            access_denied('Proforma_Invoice');
        ...
        $data['title']                = _l('Proforma_Invoice');

        $data['invoices_years']       = $this->Proforma_Invoice_model->get_invoices_years();

        $data['invoices_sale_agents'] = $this->Proforma_Invoice_model->get_sale_agents();

        $data['invoices_statuses']    = $this->Proforma_Invoice_model->get_statuses();

        $data['bodyclass']            = 'invoices-total-manual';

        $this->load->view('admin/proforma_invoice/manage', $data);
...

    {
        ...
        $data['title']                = _l('Proforma_Invoice_list_recurring');

        $data['invoices_years']       = $this->Proforma_Invoice_model->get_invoices_years();

        $data['invoices_sale_agents'] = $this->Proforma_Invoice_model->get_sale_agents();

        $this->load->view('admin/proforma_invoice/recurring/list', $data);
...

    {
     ...
     $this->app->get_table_data(($this->input->get('recurring') ? 'recurring_invoices' : 'proforma'), ['clientid' => $clientid,'data'=>$data,]);
...

    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($current_invoice != '') {
                ...
                $current_invoice_status = $this->db->get(db_prefix() . 'invoicesproforma')->row()->status;

            }



            $_data['invoices_to_merge'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != Proforma_Invoice_model::STATUS_CANCELLED) ? $this->Proforma_Invoice_model->check_for_merge_invoice($customer_id, $current_invoice) : [];



            $data['merge_info'] = $this->load->view('admin/proforma_invoice/merge_invoice', $_data, true);
            ...
            $__data['expenses_to_bill'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != Proforma_Invoice_model::STATUS_CANCELLED) ? $this->Proforma_Invoice_model->get_expenses_to_bill($customer_id) : [];



            $data['expenses_bill_info'] = $this->load->view('admin/proforma_invoice/bill_expenses', $__data, true);
...

    {
        ...
        if (total_rows('invoices', [
            ...
            'status !=' => Proforma_Invoice_model::STATUS_DRAFT,
...

    {
        ...
        if (has_permission('Proforma_Invoice', '', 'edit')) {

            $this->db->where('id', $id);

            $this->db->update(db_prefix() . 'Proforma_Invoice', ['cancel_overdue_reminders' => 1]);

        }

        redirect(admin_url('Proforma_Invoice/list_invoices/' . $id));
...

    {
        ...
        if (!has_permission('Proforma_Invoice', '', 'edit') && !has_permission('Proforma_Invoice', '', 'create')) {
        ...
        $success = $this->Proforma_Invoice_model->mark_as_cancelled($id);
...

    {
        ...
        $success = $this->Proforma_Invoice_model->unmark_as_cancelled($id);
...

    {
        ...
        if (!$id) {
            ...
            redirect(admin_url('Proforma_Invoice'));

        }

        if (!has_permission('Proforma_Invoice', '', 'create')) {

            access_denied('Proforma_Invoice');

        }

        $new_id = $this->Proforma_Invoice_model->copy($id);

        if ($new_id) {

            set_alert('success', _l('invoice_copy_success'));

            redirect(admin_url('Proforma_Invoice/invoice/' . $new_id));
        ...
        redirect(admin_url('Proforma_Invoice/invoice/' . $id));
...

    {
        ...
        $invoice = $this->Proforma_Invoice_model->get($id);
            ...
            if ($id == '') {
                ...
                if (hooks()->apply_filters('validate_invoice_number', true)) {
                    ...
                    if (total_rows('invoices', [
                        ...
                        redirect(admin_url('proforma_invoice/invoice'));
...
//print "<pre>";print_r($invoice_data); //exit;
                ...
                $id = $this->Proforma_Invoice_model->add($invoice_data);
//print "<pre>";print_r($id); exit;
                if ($id) {

                    set_alert('success', _l('added_successfully', _l('invoice')));

                    $redUrl = admin_url('proforma_invoice/list_invoices/' . $id);
                ...
                $success = $this->Proforma_Invoice_model->update($invoice_data, $id);
        ...
        if ($id == '') {
            ...
            $data['invoices_to_merge'] = $this->Proforma_Invoice_model->check_for_merge_invoice($invoice->clientid, $invoice->id);

            $data['expenses_to_bill']  = $this->Proforma_Invoice_model->get_expenses_to_bill($invoice->clientid);
            ...
            $title = _l('edit', _l('invoice_lowercase')) . ' - ' . format_proforma_number($invoice->id);
        ...
        $this->load->view('admin/proforma_invoice/invoice', $data);
			...
			foreach ($so_data as $item) {
				...
				if(count($so_data)==count($invoice_data["items"])){
					...
					if($item["id"]==$invoice_data["items"][$k]["itemid"] && $invoice_data["items"][$k]["qty"]>$item["qty"]){
						...
						redirect(admin_url('Proforma_Invoice/so_to_invoice/'.$id));
...
//print "<pre>";print_r($invoice_data);exit;
                ...
                //$ins_id = $this->Proforma_Invoice_model->add($invoice_data);

				$this->load->model('Proforma_Invoice_model');

	            $ins_id = $this->Proforma_Invoice_model->add_new_invoice($invoice_data);

                if ($ins_id) {

                    set_alert('success', _l('added_successfully', _l('invoice')));

                    $redUrl = admin_url('Proforma_Invoice/list_invoices/' . $ins_id);
...
//print "<pre>";print_r($invoice);exit;
		...
		$invoice->prefix = get_option('Proforma_invoice_prefix'); //Proforma_invoice_prefix //invoice_prefix
...

    {
        ...
        $data['invoices_to_merge']          = $this->Proforma_Invoice_model->check_for_merge_invoice($invoice->clientid, $id);

        $data['members']                    = $this->staff_model->get('', ['active' => 1]);

        $data['payments']                   = $this->payments_model->get_invoice_payments($id);

        $data['activity']                   = $this->Proforma_Invoice_model->get_invoice_activity($id);

        $data['totalNotes']                 = total_rows(db_prefix() . 'notes', ['rel_id' => $id, 'rel_type' => 'invoice']);

        $data['invoice_recurring_invoices'] = $this->Proforma_Invoice_model->get_invoice_recurring_Proforma_Invoice($id);
        ...
        $this->load->view('admin/proforma_invoice/invoice_preview_template', $data);
...

    {
        ...
        redirect(admin_url('Proforma_Invoice/list_invoices/' . $invoice_id));
...

    {
        ...
        $data['invoice']  = $this->Proforma_Invoice_model->get($id);

        $data['payments'] = $this->payments_model->get_invoice_payments($id);

        $this->load->view('admin/proforma_invoice/record_payment_template', $data);
...

    {
        ...
        if ($this->input->post()) {
            ...
            redirect(admin_url('Proforma_Invoice/list_invoices/' . $this->input->post('invoiceid')));
...

    {
        ...
        if (!$canView) {
            ...
            if (!has_permission('Proforma_Invoice', '', 'view') && !has_permission('Proforma_Invoice', '', 'view_own') && $canView == false) {
        ...
        try {
            ...
            $success = $this->Proforma_Invoice_model->send_invoice_to_client(
...

    {
        ...
        redirect(admin_url('Proforma_Invoice/list_invoices/' . $invoiceid));
...

    {
        ...
        if (!has_permission('Proforma_Invoice', '', 'delete')) {
        ...
        if (!$id) {
            ...
            redirect(admin_url('Proforma_Invoice/list_invoices'));

        }

        $success = $this->Proforma_Invoice_model->delete($id);
...

    {
        ...
        if ($file->staffid == get_staff_user_id() || is_admin()) {
            ...
            echo $this->Proforma_Invoice_model->delete_attachment($id);
...

    {
        ...
        $send = $this->Proforma_Invoice_model->send_invoice_overdue_notice($id);
...

    {
        ...
        $success = $this->Proforma_Invoice_model->set_invoice_sent($id, true);
