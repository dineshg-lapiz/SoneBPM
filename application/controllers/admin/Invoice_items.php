...
class Invoice_items extends AdminController
{
	...
	public function get_child_list($parent_id)
    {
		$html = '';
		$index = 0;
		$parent_value = $this->invoice_items_model->get_child_list($parent_id);
		//print "<pre>";print_r($parent_value);//exit;
		foreach ($parent_value as $parent) {
			$item   = $this->invoice_items_model->get($parent["sub_product_id"]);
			$item->long_description   = nl2br($item->long_description);
            $item->custom_fields_html = render_custom_fields('items', $parent_id, [], ['items_pr' => true]);
            $item->custom_fields      = [];
			
			$item->quantity = $parent["qty"];

            $cf = get_custom_fields('items');

            foreach ($cf as $custom_field) {
                $val = get_custom_field_value($parent_id, $custom_field['id'], 'items_pr');
                if ($custom_field['type'] == 'textarea') {
                    $val = clear_textarea_breaks($val);
                }
                $custom_field['value'] = $val;
