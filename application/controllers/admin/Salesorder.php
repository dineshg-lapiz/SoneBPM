...
class Salesorder extends AdminController
{
    ...
    {
        $this->list_salesorder($id);
    }

    /* List all invoices datatables */
    public function list_salesorder($id = '')
    {
        if (!has_permission('salesorder', '', 'view')
            && !has_permission('salesorder', '', 'view_own')
            && get_option('allow_staff_view_salesorder_assigned') == '0' ) { // && get_option('allow_staff_view_invoices_assigned') == '0'
            access_denied('salesorder');
        }
        ...
        $data['invoiceid']            = $id;
        $data['title']                = _l('salesorder');
        $data['invoices_years']       = $this->salesorder_model->get_invoices_years();
        $data['invoices_sale_agents'] = $this->salesorder_model->get_sale_agents();
        $data['invoices_statuses']    = $this->salesorder_model->get_statuses();
        $data['bodyclass']            = 'salesorder-total-manual';
        $this->load->view('admin/salesorder/manage', $data);
    }
    ...
    public function recurring($id = '')
    {
        if (!has_permission('salesorder', '', 'view')
            && !has_permission('salesorder', '', 'view_own')
            && get_option('allow_staff_view_salesorder_assigned') == '0') {
            access_denied('salesorder');
        }
        ...
        $data['invoices_years']       = $this->salesorder_model->get_invoices_years();
        $data['invoices_sale_agents'] = $this->salesorder_model->get_sale_agents();
        $this->load->view('admin/salesorder/recurring/list', $data);
    }
    ...
    public function table($clientid = '')
    {
        ...
        $this->app->get_table_data(($this->input->get('recurring') ? 'recurring_invoices' : 'salesorder'), [
            'clientid' => $clientid,
            'data'     => $data,
        ]);
    }
    ...
    public function client_change_data($customer_id, $current_invoice = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            $_data['invoices_to_merge'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != Invoices_model::STATUS_CANCELLED) ? $this->salesorder_model->check_for_merge_invoice($customer_id, $current_invoice) : [];

            $data['merge_info'] = $this->load->view('admin/salesorder/merge_invoice', $_data, true);

            $this->load->model('currencies_model');

            $__data['expenses_to_bill'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != Invoices_model::STATUS_CANCELLED) ? $this->salesorder_model->get_expenses_to_bill($customer_id) : [];

            $data['expenses_bill_info'] = $this->load->view('admin/salesorder/bill_expenses', $__data, true);
    ...
    public function pause_overdue_reminders($id)
    {
        if (has_permission('salesorder', '', 'edit')) {
            $this->db->where('id', $id);
            $this->db->update(db_prefix() . 'salesorder', ['cancel_overdue_reminders' => 1]);
        }
        redirect(admin_url('salesorder/list_salesorder/' . $id));
    }
    ...
    public function resume_overdue_reminders($id)
    {
        if (has_permission('salesorder', '', 'edit')) {
            $this->db->where('id', $id);
            $this->db->update(db_prefix() . 'salesorder', ['cancel_overdue_reminders' => 0]);
        }
        redirect(admin_url('salesorder/list_salesorder/' . $id));
    }
    ...
    {
        if (!has_permission('salesorder', '', 'edit') && !has_permission('salesorder', '', 'create')) {
            access_denied('salesorder');
        }

        $success = $this->salesorder_model->mark_as_cancelled($id);
    ...
    public function unmark_as_cancelled($id)
    {
        ...
        $success = $this->salesorder_model->unmark_as_cancelled($id);
        if ($success) {
    ...
    {
        if (!has_permission('salesorder', '', 'create')) {
            access_denied('invoices');
        }
        if (!$id) {
            die('No salesorder found');
        }
        $draft_invoice = false;
        ...
        $invoiceid = $this->salesorder_model->convert_to_invoice($id, false, $draft_invoice);
		
        if ($invoiceid) {
            set_alert('success', _l('salesorder_convert_to_invoice_successfully'));
	...
	public function convert_to_invoice($id)
    {
		...
		$this->session->set_userdata("so_data",$data);
	...
	public function convert_to_dc($id)
    {
        ...
        $data['bodyclass'] = 'invoice';
		$this->session->set_userdata("so_data",$data);
		redirect(admin_url('warehouse/so_to_dc/'.$id));
    }
	
    public function copy($id)
    {
        if (!$id) {
            redirect(admin_url('salesorder'));
        }
        if (!has_permission('salesorder', '', 'create')) {
            access_denied('salesorder');
        }
        $new_id = $this->salesorder_model->copy($id);
        if ($new_id) {
            set_alert('success', _l('salesorder_copy_success'));
            redirect(admin_url('salesorder/invoice/' . $new_id));
        } else {
            set_alert('success', _l('salesorder_copy_fail'));
        }
        redirect(admin_url('salesorder/invoice/' . $id));
    }
...
//print "<pre>";print_r($invoice_data);exit;
				...
				/*foreach($invoice_data["newitems"] as $item){
					print $item["available_quantity"]."<".$item["qty"];
					if($item["available_quantity"]<$item["qty"]){
						set_alert('warning', _l('Required quantity is more than Available quantity'));
						//$this->session->set_flashdata('invoice_data', $invoice_data);
						redirect(admin_url('salesorder/invoice'),'refresh');
						//break;
					}
				}*/
                ...
                if (hooks()->apply_filters('validate_invoice_number', true)) {
                    ...
                    if (total_rows('salesorder', [
                        ...
                        'number'     => $number,
                        //'status !='  => Salesorder_model::STATUS_DRAFT,
                    ])) {
                        set_alert('warning', _l('invoice_number_exists'));

                        redirect(admin_url('salesorder/invoice'));
                    }
                }
//print "<pre>";print_r($invoice_data);exit;
                $id = $this->salesorder_model->add($invoice_data);
				//print $id; exit;
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('invoice')));
                    $redUrl = admin_url('salesorder/list_salesorder/' . $id);
                ...
                $success = $this->salesorder_model->update($invoice_data, $id);
                if ($success) {
		...
		//$data = $this->session->flashdata('invoice_data');
        if ($id == '') {
            $title                  = _l('create_new_salesorder');
            $data['billable_tasks'] = [];
        } else {
            $invoice = $this->salesorder_model->get($id);

			foreach($invoice->items as $item){
				$available_qty = $this->salesorder_model->get_item_available_qty_by_name($item["description"]);
				$item["available_quantity"] = '';
				if(isset($available_qty[0]["inventory_number"])){
					$item["available_quantity"] = $available_qty[0]["inventory_number"];
				}
				$invoice_items[] = $item;
			}
            ...
            $data['invoices_to_merge'] = $this->salesorder_model->check_for_merge_invoice($invoice->clientid, $invoice->id);
            $data['expenses_to_bill']  = $this->salesorder_model->get_expenses_to_bill($invoice->clientid);

            $data['invoice']        = $invoice;
            $data['edit']           = true;
            $data['billable_tasks'] = $this->tasks_model->get_billable_tasks($invoice->clientid, !empty($invoice->project_id) ? $invoice->project_id : '');

            $title = _l('edit', _l('invoice_lowercase')) . ' - ' . format_salesorder_number($invoice->id);
        }
		...
		$data['templates'] = $this->template_list_model->get_templates('saleorder');
		
        $this->load->view('admin/salesorder/invoice', $data);
    }
	public function get_item_quantity(){
		//print "<pre>";print_r($_REQUEST);
		$commodity_no          = $_REQUEST['com_no'];
		$avail_qty = $this->salesorder_model->get_item_available_qty($commodity_no);
		//print "<pre>";print_r($avail_qty);
		echo ($avail_qty[0]['inventory_number']);
	}
    /* Get all invoice data used when user click on invoiec number in a datatable left side*/
    public function get_salesorder_data_ajax($id)
    {
        if (!has_permission('salesorder', '', 'view')
            && !has_permission('salesorder', '', 'view_own')
            && get_option('allow_staff_view_salesorder_assigned') == '0') {
        ...
        $merge_fields = array_merge($merge_fields, $this->app_merge_fields->get_flat('saleorder', 'other', '{email_signature}'));

        // Check for recorded payments
        $this->load->model('payments_model');
        $data['invoices_to_merge']          = $this->salesorder_model->check_for_merge_invoice($invoice->clientid, $id);
        $data['members']                    = $this->staff_model->get('', ['active' => 1]);
        $data['payments']                   = array();//$this->payments_model->get_invoice_payments($id);
        $data['activity']                   = $this->salesorder_model->get_invoice_activity($id);
        $data['totalNotes']                 = total_rows(db_prefix() . 'notes', ['rel_id' => $id, 'rel_type' => 'invoice']);
        $data['invoice_recurring_invoices'] = $this->salesorder_model->get_invoice_recurring_invoices($id);
...
//print "<pre>";print_r($data);exit;
		foreach($invoice->items as $item){
			...
			$available_qty = $this->salesorder_model->get_item_available_qty_by_name($item["description"]);
			$result = $this->salesorder_model->get_purchase_request($invoice->id);
			//print "<pre>";print_r($available_qty);exit;
			if(isset($result)){
				$data["create_pr"]=0;
			}else{
				if(isset($available_qty[0]["inventory_number"]) && ($available_qty[0]["inventory_number"]<$item["qty"])){
					$data["create_pr"]=1;
					/*set_alert('warning', _l('Required quantity is more than Available quantity'));
					//$this->session->set_flashdata('invoice_data', $invoice_data);
					redirect(admin_url('salesorder/invoice'),'refresh');*/
        ...
        $this->load->view('admin/salesorder/invoice_preview_template', $data);
    }
	public function mo_create_purchase_request($id)
	{
		...
		$so = $this->salesorder_model->get($id);
		$purchase_request_id = $this->salesorder_model->add_pur_request($so);
		...
		if($purchase_request_id){
			...
			$get_purchase_request = $this->salesorder_model->get_purchase_request($purchase_request_id);
    ...
    public function apply_credits($invoice_id)
    {
        ...
        redirect(admin_url('salesorder/list_salesorder/' . $invoice_id));
    }

    public function get_invoices_total()
    {
        if ($this->input->post()) {
            load_salesorder_total_template();
        }
    }
    ...
    public function record_invoice_payment_ajax($id)
    {
        ...
        $data['invoice']  = $this->salesorder_model->get($id);
        $data['payments'] = array();//$this->payments_model->get_invoice_payments($id);
        $this->load->view('admin/salesorder/record_payment_template', $data);
    }
    ...
    public function record_payment()
    {
        ...
        if ($this->input->post()) {
            ...
            redirect(admin_url('salesorder/list_salesorder/' . $this->input->post('invoiceid')));
        }
    }
    ...
    public function send_to_email($id)
    {
        ...
        if (!$canView) {
            ...
            if (!has_permission('salesorder', '', 'view') && !has_permission('salesorder', '', 'view_own') && $canView == false) {
                access_denied('salesorder');
            }
        }
        ...
        try {
            ...
            $success = $this->salesorder_model->send_invoice_to_client(
                $id,
                '',
    ...
    public function delete_payment($id, $invoiceid)
    {
        ...
        redirect(admin_url('salesorder/list_salesorder/' . $invoiceid));
    }

    /* Delete invoice */
    public function delete($id)
    {
        if (!has_permission('salesorder', '', 'delete')) {
            access_denied('salesorder');
        }
        if (!$id) {
            redirect(admin_url('salesorder/list_salesorder'));
        }
        $success = $this->salesorder_model->delete($id);
        ...
        if (strpos($_SERVER['HTTP_REFERER'], 'list_salesorder') !== false) {
            redirect(admin_url('salesorder/list_salesorder'));
        } else {
            redirect($_SERVER['HTTP_REFERER']);
        }
    ...
    public function delete_attachment($id)
    {
        ...
        if ($file->staffid == get_staff_user_id() || is_admin()) {
            echo $this->salesorder_model->delete_attachment($id);
        } else {
    ...
    public function send_overdue_notice($id)
    {
        ...
        $send = $this->salesorder_model->send_invoice_overdue_notice($id);
        if ($send) {
    ...
    public function mark_as_sent($id)
    {
        ...
        $success = $this->salesorder_model->set_invoice_sent($id, true);
