...
class Proposals extends AdminController
{
    ...
    public function proposal($id = '')
    {
		...
		$data['templates'] = $this->template_list_model->get_templates('proposals');
    ...
    public function pdf($id)
    {
		...
		foreach($proposal->items as $item){
			$com_det = $this->proposals_model->get_commodity_code_by_name($item["description"]);
			$item["commodity_code"] = $com_det[0]["commodity_code"];
			$proposal_items[] = $item;
		}
		...
		if($proposal->template_id!=""){
			$this->load->model('template_list_model');
			$proposal->templates = (array)$this->template_list_model->get($proposal->template_id);
		}
//print "<pre>";print_r($proposal);exit;
        try {
	...
	public function convert_to_salesorder($id)
    {
        ...
        if ($this->input->post()) {
            ...
            $invoice_id = $this->salesorder_model->add($this->input->post());
            if ($invoice_id) {
                set_alert('success', _l('proposal_converted_to_salesorder_success'));
                $this->db->where('id', $id);
                $this->db->update(db_prefix() . 'proposals', [
                    'saleorder_id' => $invoice_id,
                    //'status'     => 3,
                ]);
                log_activity('Proposal Converted to Saleorder [SaleOrderID: ' . $invoice_id . ', ProposalID: ' . $id . ']');
                hooks()->do_action('proposal_converted_to_invoice', ['proposal_id' => $id, 'invoice_id' => $invoice_id]);
	...
	public function get_salesorder_convert_data($id)
    {
        ...
        $this->load->view('admin/proposals/salesorder_convert_template', $data);
    }
