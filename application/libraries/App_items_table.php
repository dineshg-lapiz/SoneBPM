...
class App_items_table extends App_items_table_template
{
    ...
    public function items()
    {
        ...
        $i = 1;
		//print "<pre>";print_r($this->items);exit;
        foreach ($this->items as $item) {
            ...
            // Table data number
			$get_type_full = array_key_first($_REQUEST);
			$get_type = explode("/",$get_type_full);
			
			if($get_type[2]=="proposals" && $this->proposal_tax_per_item==0){
				$itemHTML .= '<td' . $this->td_attributes() . ' align="left" width="11%">' . $item['commodity_code'] . '</td>';
			}else{
             ...
             */
			 if($get_type[2]=="proposals" && $this->proposal_tax_per_item==0){
			 	$itemHTML .= '<td align="right" width="' . $regularItemWidth-5 . '%">' . floatVal($item['qty']);
			 }else{
            ...
            $rate = hooks()->apply_filters(
                'item_preview_rate',
                app_format_money(($item['rate']-$item['discount_amount']/$item['qty']), $this->transaction->currency_name, $this->exclude_currency()),
            ...
            $item_amount_with_quantity = hooks()->apply_filters(
                ...
                app_format_money((($item['qty'] * $item['rate'])-$item['discount_amount']), $this->transaction->currency_name, $this->exclude_currency()),
                $item,
                $this->transaction,
    ...
    public function html_headings()
    {
		...
		$get_type = explode("/",$get_type_full);
		if($get_type[2]=="proposals" && $this->proposal_tax_per_item==0){
			$html .= '<th width="10%;" align="center">' . $this->number_heading() . '</th>';
		}else{
		...
		$get_type_full = array_key_first($_REQUEST);
		$get_type = explode("/",$get_type_full);		
		if($get_type[2]=="proposals" && $this->proposal_tax_per_item==1){
    ...
    public function pdf_headings()
    {
		...
		$get_type = explode("/",$get_type_full);
		if($get_type[2]=="proposals" && $this->proposal_tax_per_item==0){
			$tblhtml .= '<th width="11%;" align="center">' . $this->number_heading() . '</th>';
		}else{
		...
		if($get_type[2]=="proposals" && $this->proposal_tax_per_item==0){
			$tblhtml .= '<th width="' . $regularItemWidth-5 . '%" align="right">' . $this->qty_heading() . '</th>';
		}else{
