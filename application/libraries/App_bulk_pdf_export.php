...
class App_bulk_pdf_export
{
     ...
     * @var array
     */
    protected $status_param_required = ['invoices','salesorder', 'estimates', 'credit_notes', 'proposals'];
    ...
    {
        $noPermissionQuery = get_salesorder_where_sql_for_staff(get_staff_user_id());
        $notSentQuery      = 'sent=0 AND status NOT IN(2,5)' . (!$this->can_view ? ' AND (' . $noPermissionQuery . ')' : '');

        $this->ci->db->select('id');
        $this->ci->db->from(db_prefix() . 'salesorder');
        if ($this->status != 'all') {
            if (is_numeric($this->status)) {
                $this->ci->db->where('status', $this->status);
            } else {
        ...
        foreach ($data as $invoice) {
            $invoice = $this->ci->salesorder_model->get($invoice['id']);
            $pdf     = invoice_pdf($invoice, $this->pdf_tag);
            $this->save_to_dir($invoice, $pdf, strtoupper(slug_it(format_salesorder_number($invoice->id))) . '.pdf');
        }
